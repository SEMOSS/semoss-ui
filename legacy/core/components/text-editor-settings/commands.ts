...
export function joinBackward(state, dispatch, view) {
    ...
    if (
        ...
        if (dispatch) {
            const tr = state.tr.deleteRange($cursor.before(), $cursor.after());
            tr.setSelection(
...
// :: (NodeType, ?Object) → (state: EditorState, dispatch: ?(tr: Transaction)) → bool
// Returns a command that tries to set the attributes of the selected textblocks
export function setNodeAttr(nodeType, attrs?) {
    return function (state, dispatch) {
        const cmd = state.tr.setNodeMarkup(nodeType, null, attrs);
        if (dispatch) {
...
// :: (MarkType, ?Object) → (state: EditorState, dispatch: ?(tr: Transaction)) → bool
// Create a command function that adds the given mark with the
// given attributes. Will return `false` when the current selection
// doesn't support that mark. If the selection is empty, this applies to the [stored
// marks](#state.EditorState.storedMarks) instead of a range of the
// document.
...
export function chainTransactions(commands) {
    return (state, dispatch): boolean => {
        ...
        return last !== undefined && last(state, dispatch); // reduced && last !== undefined && last(state, dispatch);
    };
}
...
for (const key in pcBaseKeymap) macBaseKeymap[key] = pcBaseKeymap[key];

// TODO enable mac key mapping via check for the os type. need to set global variable os
