...
jvTip.prototype.generateSimpleTip = function (dataObj, dataTable, event) {
    ...
    var offsetX = 0;
    if(event.pageX > (tip.chartDiv._groups[0][0].clientWidth / 2)) {
        offsetX = -200;
    }
    else if(event.pageX <= (tip.chartDiv._groups[0][0].clientWidth / 2)) {
        offsetX = 10;
    }
    ...
    tip.toolTip = tip.chartDiv.select(".tooltip")
        .html(function(){
            if(dataObj.viz === 'scatter' || dataObj.viz === 'treemap'){
                return generateSingleColorHTML(dataObj, dataTable, null);
            } else if(dataObj.viz === 'radial' || dataObj.viz === 'pie') {
                return generatePieHTML(dataObj, dataTable, null);
            } else if(dataObj.viz === 'jvpack' || dataObj.viz === 'jvsunburst') {
                return generatePackHTML(dataObj, dataTable, dataObj.color);
            } else {
...
function generateSingleColorHTML(dataObj, dataTable, color) {
    var tooltipText;
    tooltipText = "<div class='inline'>" + "<div class='circleBase d3-tooltip-circle inline sm-right-margin sm-left-margin' style='background: " + dataObj.color[dataObj.data[dataTable.series]] + "'>" +
...
function generatePackHTML(dataObj, dataTable, color) {
    var tooltipText;
...
function generatePieHTML(dataObj, dataTable, color) {
    var tooltipText;
