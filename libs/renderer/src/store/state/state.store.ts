...
export class StateStore {
            ...
            if (json.slots[slot]) {
                block.slots[slot] = {
                    ...
                    children: (Array.isArray(json.slots[slot])
                        ...
                        const b = this.generateBlock(child, isCommunityBlock, communityIdMap, parent);
    ...
    private addBlock = (
        ...
        if(isCommunity){
            const {newJson, variablesList} = this.buildCommunityBlockPreDeps(json);
            json = newJson;
     ...
     */
    private buildCommunityBlockPreDeps = (json: any) => {
        let newJson = json;
        let variablesList = [];
    ...
    private updateCommunityBlockPostDeps = (
        ...
        if (blockVariables.length) {
            this.updateBlockDependencyVariables(
                blockVariables,
                blockId,
            );
        }
    ...
    private updateBlockDependencyVariables = (
        ...
        blockVariables.forEach((variable) => {
            ...
            const newTo = blockIdMapper[variable.id] ||
            ...
            try {
                ...
                console.error(
                    'Error dispatching dependency variables',
                    error,
                );
            }
        });
