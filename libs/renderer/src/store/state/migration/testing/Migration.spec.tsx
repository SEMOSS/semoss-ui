...
for (const testData of testsData) {
    describe("Migration Manager starting from " + testData.name, () => {
        ...
        it("should migrate from 1.0.0-alpha.9 to 1.0.0-alpha.10 with variable.to !== query.id", async ({
            skip,
        }) => {
            const initialState = {
                queries: {
                ...
                variables: {
                    q1: { to: "q1", type: "query" },
                    "q1-cell-1": { to: "q1", type: "cell", cellId: "1" },
                    "q1-cell-2": { to: "q1", type: "cell", cellId: "2" },
                    q2: {
                        to: "q2",
                        cellId: "1",
                        type: "query",
            ...
            // skip test if the starting currentState version does not match the test version condition
            if (initialState.version !== "1.0.0-alpha.9") {
                skip();
            }
            vi.doMock("@/store/state/migration/StateVersion", () => {
                return {
            ...
            expect(initialState.version).toBe("1.0.0-alpha.9");

            const newState = await migrationManager.run(initialState);

            expect(newState).toStrictEqual(expectedState);

            expect(newState.version).toBe("1.0.0-alpha.10");
        });
    });
}
