...
import { CellState, type CellStateConfig } from "./cell.state";
import type { StateStore } from "./state.store";

export interface NotebookStateStoreInterface {
	/** Id of the query */
	id: string;
...
export class NotebookState {
	...
	private _store: NotebookStateStoreInterface = {
		id: "",
		isLoading: false,
		error: null,
		cells: {},
	...
	_update = (path: string | null, value: unknown) => {
		if (!path) {
			// set the value
			this._store = value as NotebookStateStoreInterface;
			return;
		}
