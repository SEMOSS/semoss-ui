...
type AggregateMap = Record<string, Record<string, string>>;
...
export const getSelector = (
	...
	const getAggregates = (field: keyof MapOptionFields): string => {
		if (
			!fields[field] ||
			!aggregates[field] ||
	...
	if (optionState?.fields) {
		if (label && latitude && longitude && size && color && tooltip) {
			...
			if (latitude === longitude && label === size && label === tooltip) {
				// biome-ignore lint/suspicious/noAssignInExpressions: iterative assignment pattern
