...
export const UnFilterDataCell: CellComponent<UnFilterDataCellDef> = observer(
	(props) => {
		...
		const targetCell: CellState<NotebookImportCellDef> = computed(() => {
			let c: CellState<NotebookImportCellDef> | undefined;
			let cellId: number | null = null;

			Object.values(state.notebooks).forEach((notebook) => {
				Object.entries(notebook.cells).forEach(([key, value]) => {
					const parsedId =
				...
				if (notebook.cells[cell.parameters.targetCell.id]) {
					c = notebook.cells[
						cell.parameters.targetCell.id
					] as CellState<NotebookImportCellDef>;
				}

				if (!notebook.cells[cell.parameters.targetCell.id] && cellId) {
					c = notebook.cells[
						cellId
					] as CellState<NotebookImportCellDef>;
				}
