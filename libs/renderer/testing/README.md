...
```
npx vitest AccordionBlock.spec.tsx
```

Unit testing will also run inside the pre-commit hook after linting.

To run tests via the Vitest UI, use the following script within the root project:
```
pnpm run test:browser --project renderer
```
The above will only run tests for the project specified.
