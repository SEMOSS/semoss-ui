...
interface FileNotebookRef {
	/** Refresh the notebook from its asset source. */
	refresh: () => void;
	/** Save the current notebook content. */
	save?: () => Promise<void>;
}
...
export const FileNotebook = forwardRef<FileNotebookRef, FileNotebookProps>(
	(
		...
		const rawActionsRef = useRef<React.ComponentRef<
		...
		const showRaw = isControlled ? viewMode === "raw" : internalShowRaw;
		...
		const toggleRaw = () => {
			setInternalShowRaw((shown) => !shown);
		};
		...
		useImperativeHandle(actionsRef, () => ({
			...
			save: () =>
				showRaw
					? (rawActionsRef.current?.save() ?? Promise.resolve())
					: saveNotebook(),
		}));
		...
		if (showRaw) {
			...
			return (
				<div className="relative flex h-full w-full flex-col overflow-hidden bg-background">
					<div className="flex w-full shrink-0 justify-end border-border border-b px-2 py-1">
						{!readOnly && (
							<Tooltip>
								<TooltipTrigger asChild>
									<Button
		...
		return (
			<div className="relative flex h-full w-full flex-col overflow-hidden bg-background">
				{/* Notebook execution and download remain local. */}
