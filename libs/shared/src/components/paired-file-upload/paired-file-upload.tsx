...
export function PairedFileUpload({
	...
	return (
		<div className="flex flex-col gap-1">
			...
			{rows.map((row, rowIdx) => (
				<div
					// biome-ignore lint/suspicious/noArrayIndexKey: rows have no stable id and the component manages a trailing empty row plus index-based splice removal — index identity is intentional here
					key={rowIdx}
				...
				>
					{columns.map((col) => {
						...
						return (
							// biome-ignore lint/a11y/noStaticElementInteractions: drop target — keyboard access is via the file input below
							// biome-ignore lint/a11y/useKeyWithClickEvents: clicking delegates to a focusable file input which handles keyboard activation natively
							<div
								key={col.key}
