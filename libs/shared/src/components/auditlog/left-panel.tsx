...
const TimelineRow = memo(
	({ log, isSelected, maxLat, onSelectLog }: TimelineRowProps) => {
		return (
			<button
				type="button"
				...
				<div className="relative h-2.5 flex-1 overflow-hidden rounded bg-muted/40">
					<div
						className={`absolute top-0 left-0 h-full rounded opacity-80 transition-all ${
							log.status ? "bg-primary" : "bg-destructive"
						}`}
						style={{
							width: `${Math.max(3, (log.latency / maxLat) * 100)}%`,
						}}
					/>
				</div>
				{!log.status && (
...
export const ChartPanel = memo(
	({
		...
		const maxLat = useMemo(
			() => Math.max(...logs.map((l) => l.latency), 1),
			[logs],
		);

		const chartTotalPages = useMemo(
			() => Math.ceil(logs.length / CHART_PAGE_SIZE),
			[logs.length],
		);
