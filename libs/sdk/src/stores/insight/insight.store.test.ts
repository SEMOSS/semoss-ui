...
vi.mock("../../api", () => ({
	...
	login: mockLogin,
	loginLDAP: mockLoginLDAP,
	loginOTP: mockLoginOTP,
	logout: mockLogout,
	oauth: mockOAuth,
...
describe("InsightStore", () => {
	...
	describe("actions.login()", () => {
		...
		it("ldap: calls loginLDAP API and sets isAuthorized true", async () => {
			mockLoginLDAP.mockResolvedValue(true);
			const store = new InsightStore();
			await initStore(store);
			...
			expect(mockLoginLDAP).toHaveBeenCalledWith("user", "pass");
			expect(result).toBe(true);
			expect(store.isAuthorized).toBe(true);
		});

		it("ldap: returns false when loginLDAP fails", async () => {
			mockLoginLDAP.mockResolvedValue(false);
			const store = new InsightStore();
			await initStore(store);
		...
		it("linotp: calls confirmOTP with the otp and sets isAuthorized true", async () => {
			mockConfirmOTP.mockResolvedValue(true);
			const store = new InsightStore();
			await initStore(store);
		...
		it("linotp: returns false when confirmOTP fails", async () => {
			mockConfirmOTP.mockResolvedValue(false);
			const store = new InsightStore();
			await initStore(store);
	...
	describe("actions.requestOTP()", () => {
		it("calls loginOTP with username and pin and returns true", async () => {
			mockLoginOTP.mockResolvedValue(true);
			const store = new InsightStore();

			const result = await store.actions.requestOTP("user", "1234");

			expect(mockLoginOTP).toHaveBeenCalledWith("user", "1234");
			expect(result).toBe(true);
		});

		it("returns false on UnauthorizedError", async () => {
			mockLoginOTP.mockRejectedValue(
				new UnauthorizedError("Unauthorized"),
			);
			const store = new InsightStore();

			const result = await store.actions.requestOTP("user", "bad-pin");
