...
/**
 * Ported from @semoss/shared (libs/shared/src/components/mcp/mcp-card.tsx) as
 * part of decoupling @semoss/chat from @semoss/shared — see
 * docs/chat-components/PLAN.md. Two changes from the original: (1) `useTranslation`
 * (@semoss/i18n) calls replaced with the same hardcoded English strings, since
 * @semoss/chat has no i18n infra and pulling one in just for this would
 * reintroduce a monorepo-only dependency; (2) the per-subtype brand logo
 * (`EngineSubtypeIcon`, a Vite-only asset-glob lookup) replaced by the same
 * generic `getMcpTypeIcon` icon already used for the type-label row.
 */
...
export const MCPCard = ({
	...
	const tooltipType = type === "TOOLBOX" ? "toolbox" : "knowledge store";
					...
					<div className="flex min-w-0 flex-1 items-center gap-1.5">
						{platformUrl ? (
							<Tooltip>
								...
								<TooltipContent>
									Open this {tooltipType} in the platform
								</TooltipContent>
							</Tooltip>
						) : null}
						...
						{effectivePermission === "FULLY_PRIVATE" ? (
							<Tooltip>
								...
								<TooltipContent>
									You don't have access to this {tooltipType}.
						...
						effectivePermission !== "FULLY_PRIVATE" ? (
							<Tooltip>
								...
								<TooltipContent>
									{accessMissing
										? `You don't have access to this ${tooltipType}. Please request access from the owner.`
										: `You don't have access to all dependencies of this ${tooltipType}, so functionality may be limited. Open in the platform for more information.`}
								</TooltipContent>
							</Tooltip>
						) : null}
