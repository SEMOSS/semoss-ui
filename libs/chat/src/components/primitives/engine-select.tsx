...
/**
 * Ported verbatim from @semoss/shared (libs/shared/src/components/engine/engine-select.tsx)
 * as part of decoupling @semoss/chat from @semoss/shared — see docs/chat-components/PLAN.md.
 * One real change: the per-provider brand icon (EngineSubtypeIcon, backed by a Vite-only
 * import.meta.glob() over ~145 PNG/SVG assets that plain Rollup can't process) is replaced
 * by mcp-utils.ts's already-generic, already-ported getMcpTypeIcon(engine_type) — a
 * lucide-react icon keyed by type, not per-brand logos. Confirmed with the user as an
 * acceptable tradeoff for a lean, externally-publishable package.
 */
...
interface EngineSelectPrimitiveProps {
...
/**
 * EngineSelectPrimitive - A searchable dropdown for selecting SEMOSS engines
 *
 * Features:
 ...
 */
export const EngineSelectPrimitive = ({
	className,
	disabled,
	name,
	value,
						...
						<CommandGroup>
							{getEngines.data.map((engine) => {
								...
								return (
									...
									>
										{showEngineIcon && (
											<TypeIcon className="me-2 size-4 shrink-0 text-muted-foreground" />
										)}
