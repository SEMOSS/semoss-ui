	...
	return (
		<div
			className={cn(
				...
				inline ? "relative h-full shrink-0" : "hidden",
			)}
			data-state={state}
		...
		>
			...
			<div
				...
				className={cn(
					inline
						? "relative z-10 flex h-full w-(--sidebar-width) shrink-0 transition-[width] duration-200 ease-linear"
...
function SidebarRail({ className, ...props }: React.ComponentProps<"button">) {
	...
	return (
		<button
			...
			className={cn(
				"-translate-x-1/2 group-data-[side=left]:-end-4 absolute inset-y-0 z-20 hidden w-4 transition-all ease-linear after:absolute after:inset-y-0 after:left-1/2 after:w-[2px] hover:after:bg-sidebar-border group-data-[side=right]:start-0 sm:flex",
