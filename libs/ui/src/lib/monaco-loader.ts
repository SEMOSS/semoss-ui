...
// Wrap the @monaco-editor/react Editor/DiffEditor so their wrapper div always
// renders with `dir="ltr"`. Monaco assumes left-to-right for code (the
// textarea, line numbers, completion popups, hit-testing of mouse clicks all
// break under `dir="rtl"`). Forcing LTR at the wrapper keeps the editor
// usable inside an RTL page (Arabic/Hebrew). Callers can still override via
// `wrapperProps`.
...
/** Lazy-loaded `@monaco-editor/react` `Editor`, with Monaco workers/loader wired up. */
...
/** Lazy-loaded `@monaco-editor/react` `DiffEditor`, with Monaco workers/loader wired up. */
export const LazyMonacoDiffEditor = React.lazy(() =>
	ensureMonacoSetup()
		.then(() => loadMonacoReact())
		.then((mod) => ({
