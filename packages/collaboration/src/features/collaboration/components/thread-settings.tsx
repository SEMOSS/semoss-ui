...
export function ThreadSettings({ thread }: { thread: Thread }) {
	const fieldId = useId();
		...
		<div className="space-y-6">
			...
			<Section title="People">
				...
				<Small className="text-muted-foreground">
					...
					questions and remains subject to Sources and rules. Stored
					conversations and quoted text in other messages are not
					deleted.
				</Small>
			</Section>
			<Section title="Work visibility">
				<div className="flex items-center justify-between gap-3">
					<Label htmlFor={`${fieldId}-mute`}>Mute this thread</Label>
					<Switch
						id={`${fieldId}-mute`}
				...
				<Small className="text-muted-foreground">
					Muted threads remain in Brain and are hidden from the Work
					feed.
				</Small>
			</Section>
		</div>
