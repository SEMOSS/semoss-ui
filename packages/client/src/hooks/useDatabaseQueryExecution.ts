...
function detectQueryType(query: string): string {
    ...
    } else return ast[0].type.toUpperCase() as 'SELECT' | 'OTHER';
}
...
export const isErrorResponse = (response: any): boolean => {
    return (
        response?.operationType?.includes('ERROR') ||
        (typeof response?.output === 'string' &&
            response.output.startsWith('ERROR:'))
    );
};
...
export function useQueryExecution(
    ...
    const executeQuery = async () => {
        ...
        try {
            ...
            let pixel = `SqlQuery(database=["${engineId}"], query=["<encode>${removeComments(
                query,
            ).replaceAll(
                '`',
                '',
            )}</encode>"], limit=[${limit}], commit = [true]);`;
