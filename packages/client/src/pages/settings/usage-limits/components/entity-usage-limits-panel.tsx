...
interface EntityUsageLimitsPanelProps {
	entityType: "MODEL" | "DATABASE" | "VECTOR" | "STORAGE" | "APP" | "AGENT";
	entityId: string;
	entityName: string;
}

/**
 * Reusable panel for configuring per-user and per-team token limits
 * on a specific entity (model, database, vector, storage, app, or agent).
 * Currently uses mocked data — will be replaced with API calls.
 */
export function EntityUsageLimitsPanel({
		...
		limits.map((limit) => (
			...
			>
				<div className="flex items-center gap-2">
					<Label className="whitespace-nowrap text-xs">Total:</Label>
					<Input
						type="number"
	...
	return (
		<div
			...
			data-testid="entity-usage-limits-panel"
		>
			{/* Header */}
			<div>
				<h2 className="font-semibold text-lg">
					Usage Limits — {entityName}
				</h2>
			...
			<section>
				<div className="mb-3 flex items-center justify-between">
					<div>
						...
						<p className="text-muted-foreground text-sm">
							Default token limits for each user on this{" "}
							{entityTypeLabel.toLowerCase()}.
						</p>
					</div>
					<Button
			...
			<section>
				<div className="mb-3 flex items-center justify-between">
					<div>
						...
						<p className="text-muted-foreground text-sm">
							Shared team token pool on this{" "}
							{entityTypeLabel.toLowerCase()}.
						</p>
					</div>
					<Button
				...
				<ExceptionsSection
					...
					renderEntityDetails={(team) => (
						<EntityDetailRow
							primary={team.name}
							details={[
								{ label: "Type", value: String(team.teamType) },
								{
									label: "Members",
