...
export const FunctionForm = ({ id, title, description, fields }) => {
  ...
  const onFormSubmit = async (formData) => {
    ...
    try {
        ...
        if (title === "ZIP") {
          ...
          navigate(`/engine/functions/${uploadEngineOutput.database_id}`);
          return;
        }
      ...
      const createFunctionOutput = createFunctionResponse.pixelReturn[0].output;
      const createFunctionOperationType =
        createFunctionResponse.pixelReturn[0].operationType;

      if (createFunctionOperationType.includes("ERROR")) {
        notification.add({
          color: "error",
          message: createFunctionOutput,
        });
        return;
      }
      ...
      notification.add({
        color: "success",
        message: `Successfully added functions database to catalog`,
      });

      const functionsDatabaseId = createFunctionOutput.database_id;

      if (functionsFormValues.secondaryFields["EMBEDDINGS"]) {
        const uploadedEmbeddingFiles = await monolithStore.uploadFile(
          functionsFormValues.secondaryFields["EMBEDDINGS"],
          configStore.store.insightID
        );
        ...
        notification.add({
          color:
            embeddingsResponse.pixelReturn?.[0]?.operationType?.indexOf(
              ...
              ? "error"
              : "success",
          message: "Failed to add embeddings to functions database",
        });
        notification.add({
          color: "success",
          message: `Successfully added functions database to catalog`,
      ...
      if (functionsFormValues.fields["DESCRIPTION"]) {
        engineMetadata["description"] = functionsFormValues.fields["DESCRIPTION"];
      }
      if (functionsFormValues.fields["TAGS"]) {
        engineMetadata["tag"] = functionsFormValues.fields["TAGS"];
      }
      ...
      navigate(`/engine/functions/${functionsDatabaseId}`);
    } catch (error) {
      notification.add({
        color: "error",
          ...
          case "text-field":
            return (
              <TextField
                ...
                data-testid={`functions-form-input-${val.fieldName}`}
              />
            );
              ...
              >
                {val?.options?.options?.map((opt) => (
                  <Menu.Item
                    key={opt.value}
                    value={opt.value}
                    data-testid={`functions-form-option-${val.fieldName}-${opt.value}`}
                  >
                    {opt.display}
                  </Menu.Item>
                ))}
              ...
              >
                {val.options.options.map((opt) => (
                  <FormControlLabel
                    ...
                    control={
                      <Radio
                        data-testid={`functions-form-radio-${val.fieldName}-${opt.value}`}
                      />
                    }
                    label={opt.display}
                  />
              ...
              <>
                ...
                {error && (
                  <Typography
                    variant="body1"
                    color="red"
                    data-testid={`functions-form-error-${val.fieldName}`}
                  >
                    {getHelperText(error, val)}
                  </Typography>
                )}
  ...
  return (
    <form onSubmit={handleSubmit(onFormSubmit)} data-testid="functions-form">
      <Typography variant="h4" data-testid="functions-form-title">
        {title}
      </Typography>
      <Typography variant="body1" data-testid="functions-form-description">
        {description}
      </Typography>

      <StyledBox data-testid="functions-form-box">
        <Stack rowGap={4}>
          {SECTION_ORDER.map((sectionKey) => {
            ...
            return (
              <div
                data-testid={`functions-form-section-${sectionKey}`}
                key={sectionKey}
              >
                <SectionContainer container spacing={2}>
                  <SectionLeft>
                    <Typography
                      variant="h6"
                      gutterBottom
                      data-testid={`functions-form-section-title-${sectionKey}`}
                    >
                    ...
                    <Typography
                      variant="body2"
                      color="textSecondary"
                      data-testid={`functions-form-section-desc-${sectionKey}`}
                    >
                      {sectionDesc}
                    </Typography>
                  </SectionLeft>
                  <SectionRight
                    data-testid={`functions-form-section-fields-${sectionKey}`}
                  >
                    {sectionFields.map((val) => (
                      <div
                        key={val.fieldName}
                        data-testid={`functions-form-field-${val.fieldName}`}
                      >
                ...
                <Divider
                  data-testid={`functions-form-section-divider-${sectionKey}`}
                />
              </div>
            );
          })}
          ...
          {defaultFields.filter((f) => !f.section).length > 0 && (
            <SectionContainer data-testid="functions-form-no-section">
              {defaultFields
                .filter((f) => !f.section)
                .map((val) => (
                  <StyledNoSection
          ...
          {advancedFields.length ? (
            <>
              <AdvancedHeader data-testid="functions-form-advanced-header">
                <Typography variant="body1">ADVANCED SETTINGS</Typography>
                <IconButton
                  onClick={() => setOpenAdvanced(!openAdvanced)}
                  data-testid="functions-form-advanced-toggle"
                >
        ...
        <StyledFlexEnd data-testid="functions-form-actions">
          <StyledSubmitButton
            type="submit"
            variant="contained"
            data-testid="functions-form-submit"
          >
            Create Function
          </StyledSubmitButton>
        </StyledFlexEnd>
