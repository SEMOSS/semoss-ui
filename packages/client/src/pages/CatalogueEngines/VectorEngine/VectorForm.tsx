...
export const VectorForm = ({ title, description, fields }) => {
	...
	const executeWatchedFieldPixel = useCallback(
		...
		[monolithStore, notification, setValue],
	);

	const hasParameterizedValue = useCallback(
		(str) => /<([^>]+)>/.test(str),
		[],
	);
	...
	const onFormSubmit = async (formData) => {
		...
		try {
          ...
          engineTypes=["VECTOR"]
				...
				const uploadEngineResponse =
					await monolithStore.runQuery(uploadEnginePixel);
				const uploadEngineOutput =
					uploadEngineResponse.pixelReturn[0].output;
				...
				navigate(`/engine/vector/${uploadEngineOutput.database_id}`);
				return;
			}
			...
			const createVectorPixel = `CreateVectorDatabaseEngine(
      database=["${vectorFormValues.name}"], 
      conDetails=[${JSON.stringify(vectorFormValues.fields)}]
    );`;

			const createVectorResponse =
				await monolithStore.runQuery(createVectorPixel);
			const createVectorOutput =
				createVectorResponse.pixelReturn[0].output;
			const createVectorOperationType =
				createVectorResponse.pixelReturn[0].operationType;

			if (createVectorOperationType.includes("ERROR")) {
				notification.add({
					color: "error",
					message: createVectorOutput,
				});
				return;
			}
			...
			const vectorDatabaseId = createVectorOutput.database_id;

			if (vectorFormValues.secondaryFields["EMBEDDINGS"]) {
				const uploadedEmbeddingFiles = await monolithStore.uploadFile(
					vectorFormValues.secondaryFields["EMBEDDINGS"],
					configStore.store.insightID,
				);
        ...
        filePaths=${JSON.stringify(embeddingFileLocations)}
			...
			if (vectorFormValues.fields["DESCRIPTION"]) {
				engineMetadata["description"] =
					vectorFormValues.fields["DESCRIPTION"];
			}
			if (vectorFormValues.fields["TAGS"]) {
				engineMetadata["tag"] = vectorFormValues.fields["TAGS"];
			}
        ...
        jsonCleanup=[true]
				...
				const metadataResponse =
					await monolithStore.runQuery(metadataPixel);
				...
				notification.add({
					color: metadataOperationType.includes("ERROR")
						? "error"
						: "success",
			...
			navigate(`/engine/vector/${vectorDatabaseId}`);
		} catch (error) {
			notification.add({
				color: "error",
					...
					case "text-field":
						return (
							<TextField
								...
								data-testid={`vector-form-input-${val.fieldName}`}
							/>
						);
							...
							>
								{val?.options?.options?.map((opt) => (
									<Menu.Item
										key={opt.value}
										value={opt.value}
										data-testid={`vector-form-option-${val.fieldName}-${opt.value}`}
									>
										{opt.display}
									</Menu.Item>
								))}
							...
							>
								{val.options.options.map((opt) => (
									<FormControlLabel
										...
										control={
											<Radio
												data-testid={`vector-form-radio-${val.fieldName}-${opt.value}`}
											/>
										}
										label={opt.display}
									/>
							...
							<>
								...
								{error && (
									<Typography
										variant="body1"
										color="red"
										data-testid={`vector-form-error-${val.fieldName}`}
									>
										{getHelperText(error, val)}
									</Typography>
								)}
	...
	return (
		<form onSubmit={handleSubmit(onFormSubmit)} data-testid="vector-form">
			...
			<Typography variant="body1" data-testid="vector-form-description">
				{description}
			</Typography>
			...
			<StyledBox data-testid="vector-form-box">
				<Stack rowGap={4}>
					{SECTION_ORDER.map((sectionKey) => {
						...
						return (
							<div
								data-testid={`vector-form-section-${sectionKey}`}
								key={sectionKey}
							>
								<SectionContainer container spacing={2}>
									<SectionLeft>
										<Typography
											variant="h6"
											gutterBottom
											data-testid={`vector-form-section-title-${sectionKey}`}
										>
										...
										<Typography
											variant="body2"
											color="textSecondary"
											data-testid={`vector-form-section-desc-${sectionKey}`}
										>
											{sectionDesc}
										</Typography>
									</SectionLeft>
									<SectionRight
										data-testid={`vector-form-section-fields-${sectionKey}`}
									>
										{sectionFields.map((val) => (
											<div
												key={val.fieldName}
												data-testid={`vector-form-field-${val.fieldName}`}
											>
								...
								<Divider
									data-testid={`vector-form-section-divider-${sectionKey}`}
								/>
							</div>
						);
					})}
					...
					{defaultFields.filter((f) => !f.section).length > 0 && (
						<SectionContainer data-testid="vector-form-no-section">
							{defaultFields
								.filter((f) => !f.section)
								.map((val) => (
									<StyledNoSection
