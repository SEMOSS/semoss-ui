...
export const DatabaseForm = ({ id, title, description, fields }) => {
  ...
  const updateStepBasedOnMetaModel = (METAMODEL_TYPE) => {
    if (METAMODEL_TYPE === "asFlatTable" || METAMODEL_TYPE === "fromScratch") {
      setStep("table");
...
useEffect(() => {
        ...
        if (databaseType?.toLowerCase() === "r") {
          ...
          return {
            ...
            options: {
              ...f.options,
              options: fields.find(
                (orig) => orig.fieldName === "METAMODEL_TYPE"
  ...
  const onFormSubmit = async (formData) => {
    ...
    try {
      ...
      for (const pixelString of pixelExpressions) {
        ...
        const filePathFromExpression = filePathMatch ? filePathMatch[1] : null;
  ...
  return (
    <>
      {step === "fileupload" && (
        <form onSubmit={handleSubmit(onFormSubmit)} data-testid="database-form">
          <Typography variant="h4" data-testid="database-form-title">
            {title}
          </Typography>
          <Typography variant="body1" data-testid="database-form-description">
            {description}
          </Typography>
      ...
      {step === "table" && parsedData && parsedData.length > 0 && (
        <DataSelection
          files={parsedData}
          fileName={fileName}
          onImport={(payload) => submitTablePixel(payload,formValues)}
      ...
      {step === "metaModel" && parsedData && parsedData.length > 0 && (
        <MetaModelType
          ...
          onImport={() => submitMetamodelPixel(parsedData,formValues)}
          onCancel={handleCancel}
        />
      )}
