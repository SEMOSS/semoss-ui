...
export const ModelForm = ({ id, title, description, fields }) => {
  ...
  const onFormSubmit = async (formData) => {
    ...
    try {
        ...
        if (title === "ZIP") {
          ...
          navigate(`/engine/model/${uploadEngineOutput.database_id}`);
          return
        }
      ...
      const createModelPixel = `CreateModelEngine(
                    model=["${vectorFormValues.name}"], 
                    modelDetails=[${JSON.stringify(vectorFormValues.fields)}]
                )`;

    
      const createModelResponse = await monolithStore.runQuery(
        createModelPixel
      );
      const createModelOutput = createModelResponse.pixelReturn[0].output;
      const createModelOperationType =
        createModelResponse.pixelReturn[0].operationType;

      if (createModelOperationType.includes("ERROR")) {
        notification.add({
          color: "error",
          message: createModelOutput,
        });
        return;
      }
      ...
      notification.add({
        color: "success",
        message: `Successfully added model database to catalog`,
      });

      const vectorDatabaseId = createModelOutput.database_id;
        ...
        notification.add({
          color:
            embeddingsResponse.pixelReturn?.[0]?.operationType?.indexOf(
              ...
              ? "error"
              : "success",
          message: "Failed to add embeddings to model database",
        });
        notification.add({
          color: "success",
          message: `Successfully added model database to catalog`,
      ...
      if (vectorFormValues.fields["DESCRIPTION"]) {
        engineMetadata["description"] = vectorFormValues.fields["DESCRIPTION"];
      }
      ...
      navigate(`/engine/model/${vectorDatabaseId}`);
    } catch (error) {
      notification.add({
        color: "error",
          ...
          case "text-field":
            return (
              <TextField
                ...
                data-testid={`model-form-input-${val.fieldName}`}
              />
            );
              ...
              >
                {val?.options?.options?.map((opt) => (
                  <Menu.Item
                    key={opt.value}
                    value={opt.value}
                    data-testid={`model-form-option-${val.fieldName}-${opt.value}`}
                  >
                    {opt.display}
                  </Menu.Item>
                ))}
              ...
              >
                {val.options.options.map((opt) => (
                  <FormControlLabel
                    ...
                    control={
                      <Radio
                        data-testid={`model-form-radio-${val.fieldName}-${opt.value}`}
                      />
                    }
                    label={opt.display}
                  />
              ...
              <>
                ...
                {error && (
                  <Typography
                    variant="body1"
                    color="red"
                    data-testid={`model-form-error-${val.fieldName}`}
                  >
                    {getHelperText(error, val)}
                  </Typography>
                )}
  ...
  return (
    <form onSubmit={handleSubmit(onFormSubmit)} data-testid="model-form">
      <Typography variant="h4" data-testid="model-form-title">
        {title}
      </Typography>
      <Typography variant="body1" data-testid="model-form-description">
        {description}
      </Typography>

      <StyledBox data-testid="model-form-box">
        <Stack rowGap={4}>
          {SECTION_ORDER.map((sectionKey) => {
            ...
            return (
              <div
                data-testid={`model-form-section-${sectionKey}`}
                key={sectionKey}
              >
                <SectionContainer container spacing={2}>
                  <SectionLeft>
                    <Typography
                      variant="h6"
                      gutterBottom
                      data-testid={`model-form-section-title-${sectionKey}`}
                    >
                    ...
                    <Typography
                      variant="body2"
                      color="textSecondary"
                      data-testid={`model-form-section-desc-${sectionKey}`}
                    >
                      {sectionDesc}
                    </Typography>
                  </SectionLeft>
                  <SectionRight
                    data-testid={`model-form-section-fields-${sectionKey}`}
                  >
                    {sectionFields.map((val) => (
                      <div
                        key={val.fieldName}
                        data-testid={`model-form-field-${val.fieldName}`}
                      >
                ...
                <Divider
                  data-testid={`model-form-section-divider-${sectionKey}`}
                />
              </div>
            );
          })}
          ...
          {defaultFields.filter((f) => !f.section).length > 0 && (
            <SectionContainer data-testid="model-form-no-section">
              {defaultFields
                .filter((f) => !f.section)
                .map((val) => (
                  <StyledNoSection
          ...
          {advancedFields.length ? (
            <>
              <AdvancedHeader data-testid="model-form-advanced-header">
        ...
        <StyledFlexEnd data-testid="model-form-actions">
          <StyledSubmitButton
            type="submit"
            variant="contained"
