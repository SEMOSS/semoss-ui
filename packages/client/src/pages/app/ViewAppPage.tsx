  ...
  const handleBookmark = (status: boolean) => {
    ...
    setProjectFavorite(appId, status)
      .then(() => {
        notification.add({
          color: "success",
          message: `Project ${bookmarked ? "unbookmarked" : "bookmarked"}`,
        });
        return;
      })
      .catch((err) => {
  ...
  useEffect(() => {
    ...
    configStore
      ...
      .then((loadedWorkspace) => {
        ...
        setBookmarked(Boolean(loadedWorkspace.metadata.project_favorite));
      })
      .catch((e) => {
        notification.add({
          color: "error",
    ...
    <>
      ...
      <NavbarRight>
        ...
        <Button
          ...
          size={"small"}
          color="primary"
          disabled={!(workspace.role === "OWNER" || workspace.role === "EDIT")}
      ...
      <StyledContent>
        ...
        {workspace.type === "CODE" ? <CodeRenderer appId={appId} /> : null}
      </StyledContent>

      <Modal open={isShareOpen} onClose={() => setIsShareOpen(false)}>
        <ShareOverlay appId={appId} onClose={() => setIsShareOpen(false)} />
      </Modal>
    </>
  );
});
