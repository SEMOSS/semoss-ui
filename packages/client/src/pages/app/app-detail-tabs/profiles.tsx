import { Pencil, Settings2, Trash2, UserPlus, Users, X } from "lucide-react";
...
export const AppProfiles = ({ appId }: AppProfilesProps) => {
	...
	// Manage subgroup dialog state
	const [managingSubgroup, setManagingSubgroup] = useState<Subgroup | null>(
		null,
	);
	...
	const [assigningSubgroupUser, setAssigningSubgroupUser] = useState(false);
	...
	useEffect(() => {
		if (managingSubgroup) {
			loadSubgroupFeatures(managingSubgroup.subgroupId);
			loadSubgroupUsers(managingSubgroup.subgroupId);
		}
	}, [managingSubgroup]);
	...
	function closeManageSubgroupDialog() {
	...
	async function handleSaveSubgroup() {
		const name = sanitizeForPixel(subgroupFormName.trim());
		if (!name) {
			toast.error("Subgroup name is required.");
			return;
		}
		if (!selectedProfile) return;
		const desc = sanitizeForPixel(subgroupFormDesc);
	...
	async function handleToggleSubgroupFeature(feature: Feature) {
		if (!managingSubgroup) return;
		await runPixel(
			`SetAppSubgroupFeature(app="${appId}", subgroupId="${managingSubgroup.subgroupId}", featureId="${feature.featureId}", enabled="${!feature.enabled}");`,
		);
		await loadSubgroupFeatures(managingSubgroup.subgroupId);
	}

	async function handleAssignSubgroupUser() {
		if (!managingSubgroup || !assignSubgroupUser) return;
		setAssigningSubgroupUser(true);
		try {
			await runPixel(
				`AssignAppUserSubgroup(app="${appId}", userId="${assignSubgroupUser.id}", subgroupId="${managingSubgroup.subgroupId}");`,
			);
			await loadSubgroupUsers(managingSubgroup.subgroupId);
	...
	async function confirmRemoveSubgroupUser() {
		if (!removeSubgroupUserTarget || !managingSubgroup) return;
		setConfirmLoading(true);
		try {
			await runPixel(
				`RemoveAppUserSubgroup(app="${appId}", userId="${removeSubgroupUserTarget.userId}", subgroupId="${managingSubgroup.subgroupId}");`,
			);
			await loadSubgroupUsers(managingSubgroup.subgroupId);
	...
	return (
		<div className="flex h-full gap-4 p-1">
					...
					{selectedProfile.isGroup ? (
						...
						>
							...
							>
								<div className="flex items-center justify-between">
									<p className="text-muted-foreground text-sm">
										Base features enabled for all members of
										this group profile.
									</p>
									<Button
										size="sm"
							...
							>
								<div className="flex items-center justify-between">
									<p className="text-muted-foreground text-sm">
										Subgroups within this group profile.
									</p>
									<Button
										size="sm"
										variant="outline"
													...
													<TableCell>
														<div className="flex items-center gap-1">
															<Button
																...
																data-testid={`manage-subgroup-${sg.subgroupId}`}
															>
						...
						>
							...
							>
								<div className="flex items-center justify-between">
									<p className="text-muted-foreground text-sm">
										Toggle which features are enabled for
										this profile.
									</p>
									<Button
										size="sm"
							...
							>
								<div className="flex items-center justify-between">
									<p className="text-muted-foreground text-sm">
										Users explicitly assigned to this
										profile.
									</p>
									<Button
										size="sm"
						...
						<div className="flex items-center justify-between">
							<div>
								...
								<DialogDescription className="mt-1 text-xs">
									Managers can view profiles and assign users,
									but cannot create profiles, features, or
									subgroups.
								</DialogDescription>
							</div>
							<Button
						...
						{!editingProfile && (
							<div className="flex items-center gap-2">
								...
								<Label
									...
									className="cursor-pointer"
								>
									Group profile — contains named sub-groups
								</Label>
							</div>
						)}
					</div>
						...
						<div className="flex flex-col gap-1.5">
							...
							<Input
								...
								onChange={(e) => {
									...
									setFeatureKeyError(
										validateFeatureKey(e.target.value) ||
											!e.target.value
											? ""
					...
					<DialogFooter>
						<div className="flex flex-row gap-2">
							...
							<Button
								...
								disabled={
									savingSubgroup || !subgroupFormName.trim()
								}
			...
			{/* ── Manage Subgroup dialog ───────────────────────────────────── */}
			<Dialog
				open={!!managingSubgroup}
				onOpenChange={(isOpen) =>
					!isOpen && closeManageSubgroupDialog()
				}
			>
				<DialogContent
					className="max-w-[640px] gap-4 rounded-xl"
					showCloseButton={false}
				>
					<DialogHeader>
						<div className="flex items-center justify-between">
							<DialogTitle>
								Manage {managingSubgroup?.subgroupName}
							</DialogTitle>
							<Button
								variant="ghost"
								size="icon-sm"
								onClick={closeManageSubgroupDialog}
								className="hover:bg-accent"
							>
								<X className="size-4" />
							</Button>
					...
					>
						...
						>
							<div className="flex items-center justify-between">
								<p className="text-muted-foreground text-sm">
									Users assigned to this subgroup.
								</p>
								<Button
									size="sm"
									variant="outline"
									onClick={() =>
										setShowAssignSubgroupUserModal(true)
									}
							...
							{subgroupUsers.length === 0 ? (
								<p className="text-muted-foreground text-sm">
									No users assigned to this subgroup.
								</p>
							) : (
								<Table>
									<TableHeader>
			...
			>
				...
				>
					<DialogHeader>
						<div className="flex items-center justify-between">
							<DialogTitle>Assign User to Subgroup</DialogTitle>
							<Button
								variant="ghost"
								size="icon-sm"
								onClick={() => {
					...
					<div className="flex flex-col gap-4 pb-2">
						<div className="flex flex-col gap-1.5">
							...
							<UserSearchCombobox
								...
								excludeIds={subgroupUsers.map((u) => u.userId)}
					...
					<DialogFooter>
						<div className="flex flex-row gap-2">
							...
							<Button
								...
								disabled={
									assigningSubgroupUser || !assignSubgroupUser
								}
				...
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>
							...
							&quot; from this profile? They will be removed from
							this profile assignment. They may still be in other
							profiles.
						</DialogDescription>
					</DialogHeader>
					<DialogFooter>
			...
			<Dialog
				...
				onOpenChange={(open) => !open && setDeleteSubgroupTarget(null)}
			>
				<DialogContent>
					<DialogHeader>
				...
				<DialogContent>
					<DialogHeader>
						...
						<DialogDescription>
							...
							&quot; from this subgroup? They will be removed from
							this subgroup assignment. They may still be in other
							profiles.
						</DialogDescription>
					</DialogHeader>
					<DialogFooter>
						<Button
							variant="ghost"
							onClick={() => setRemoveSubgroupUserTarget(null)}
							disabled={confirmLoading}
						>
							Cancel
						</Button>
