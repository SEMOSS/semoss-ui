...
export const EngineQAPage = () => {
    ...
    const ask = handleSubmit(async (data: { QUESTION: string }) => {
        ...
        try {
            ...
            pixel = `
            LLM(engine="${selectedModel.database_id}" , command=["<encode>You are an intelligent AI designed to answer notebooks based on provided policy documents. If an answer cannot be determined based on the provided policy documents, inform the user. Answer as truthfully as possible at all times and tell the user if you do not know the answer. Please be concise and get to the point. Here is the question: ${data.QUESTION}. Here are the policy documents: ${contextDocs}</encode>"], paramValues=[{"temperature":${temperature}}])
            `;
