import type { Block, QueryState, StateStore } from "@semoss/renderer";

type ReplaceSection = "blocks" | "queries" | "variables" | "all" | "";
...
const rewriteListenerPayloads = (
	...
	Object.values(listeners).forEach((listener) => {
		...
		listener.order.forEach((orderItem) => {
			...
			// Query-targeted rewrite (RunQueryAction | RunCellAction)
			if (
				tgtQ &&
				!tgtC &&
				"queryId" in payload &&
...
const replaceInVariable = (
	...
	if (fromPattern) {
		next.variables![variableId] = walkAndReplace(v, fromPattern, toPattern);
	}
...
const replaceInQuery = (
	...
	if (!state.queries?.[queryKey]) return state;
	const next = deepClone(state);
	const query = next.queries![queryKey];
...
export function replaceDependentReferences(
	...
	const applyByKey = (key: string, value: string) => {
		...
		if (isQueries && next.queries?.[key]) {
			next = replaceInQuery(next, key, targets, value);
			return;
		}
		...
		} else if (replaceSection === "queries" && next.queries?.[key]) {
			next = replaceInQuery(next, key, targets, value);
