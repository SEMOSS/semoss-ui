...
export class ConfigStore {
  ...
  get theme(): {
    ...
    let customTheme = {};
    try {
      if (this._store.config.theme && this._store.config.theme["THEME_MAP"]) {
  ...
  isEngineOperationAvailable = (
    ...
    return (
      this._store.config[`adminOnly${moduleMap[type]}${operationMap[flag]}`] ===
      false
    );
  };
  ...
  async createWorkspace(appId: string) {
    ...
    const getUserProjectPermission = await getUserProjectLevelPermission(appId);
    ...
    // get the metadata
    const getAppInfo = await this._root.monolithStore.runQuery<[AppMetadata]>(
