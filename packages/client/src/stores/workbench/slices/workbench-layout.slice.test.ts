...
describe("workbench layout cache", () => {
	...
	it("persists under the id-scoped key on first load", () => {
		...
		expect(store.getState().cacheKey).toBe("smss-workbench--proj-1-v2");
		expect(
			JSON.parse(
				localStorage.getItem("smss-workbench--proj-1-v2") as string,
			),
	...
	it("isolates instances by id, so view/share surfaces get their own cache", () => {
		...
		expect(
			localStorage.getItem("smss-workbench--proj-1-view-v2"),
		).toBeTruthy();
	});

	it("falls back to the default on unparseable JSON", () => {
		localStorage.setItem("smss-workbench--proj-1-v2", "{");
