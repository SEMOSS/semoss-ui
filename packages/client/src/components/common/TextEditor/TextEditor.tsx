...
export const TextEditor = (props: TextEditorProps) => {
    ...
    const [LLMStarterModal, setLLMStarterModal] = useState(false);
    const [LLMLoading, setLLMLoading] = useState(false);

    // useEffect(() => {
    //     alert("activeIndex change")

    //     const af = controlledFiles[activeIndex];
    //     console.log({activeIndex, af})
    //     if (af && af.content == "") {
    //         alert("Open LLM starter code modal");
    //         alert("Empty file. Would you like to populate with starter code?");
    //     }
    ...
    const _createStarterCode = async () => {
        const promptString = `Generate starter, boilerplate code for a file with the file name "${activeFile.name}". Be comprehensive. Include professional, clear, consise explanatory comments explaining each section of code. Include a leading comment explaining that this is starter code generated by a Large Language Model and that it will need to be modified to work correctly in the users application. Also remind the user that they will have to save the file for this code to be saved. Assume the user is not a professional software engineer. Include links to helpful documentation.`;
        ...
        try {
            ...
            // const trimmedStarterCode = LLMResponse.slice(3, LLMResponse.length - 3); // trims off triple quotes

            // activeFile.content = trimmedStarterCode;
            activeFile.content = LLMResponse;
            setLLMLoading(false);
    ...
    const createStarterCode = async () => {
        // const promptString = `Generate starter, boilerplate code for a file with the file name "${activeFile.name}". Be comprehensive. Include professional, clear, consise explanatory comments explaining each section of code. Include a leading comment explaining that this is starter code generated by a Large Language Model and that it will need to be modified to work correctly in the users application. Also remind the user that they will have to save the file for this code to be saved. Assume the user is not a professional software engineer. Include links to helpful documentation.`;
        // const promptString = `Generate starter, boilerplate code for a file with the file name '${activeFile.name}'. Be comprehensive. Include professional, clear, consise explanatory comments explaining each section of code. Include a leading comment explaining that this is starter code generated by a Large Language Model and that it will need to be modified to work correctly in the users application. Also remind the user that they will have to save the file for this code to be saved. Assume the user is not a professional software engineer. Include links to helpful documentation.`;
        const promptString = `Generate starter, boilerplate code for a file with the file name '${activeFile.name}'. Include consise explanatory comments.`;
        let pixel = `LLM(engine = "3def3347-30e1-4028-86a0-83a1e5ed619c", command = "${promptString}", paramValues = [ {} ] );`;

        try {
            const res = await runPixel(pixel);
            const LLMResponse = res.pixelReturn[0].output.response;
            let trimmedStarterCode = LLMResponse.slice(
                3,
                LLMResponse.length - 3,
            ); // trims off triple quotes
    ...
    const activeFile = useMemo<ControlledFile | null>(() => {
        const af = controlledFiles[activeIndex];
        if (af && af.content.length < 1) {
            // open LLM starter modal
            setLLMStarterModal(true);
            // af.content = LLMStarterCode;
        }
        if (af) return af;
        return null;
