...
const StyledFormTypeStorageBox = styled(Box, {
...
const StyledInnerBox = styled("div")<{ isStorage?: boolean }>(
	({ theme, isStorage }) => ({
		display: "flex",
		alignItems: isStorage ? "flex-start" : "center",
		gap: theme.spacing(1),
		flexDirection: isStorage ? "column" : "row",
	}),
);

// Replaces image with a colored avatar containing initials
const StyledStorageAvatar = styled("div")<{ gradientBg: string }>(
	({ gradientBg }) => ({
		display: "flex",
		height: "40px",
		width: "40px",
...
const StyledCardStorageText = styled("p")({
	overflow: "hidden",
...
export const StorageTitleCard: React.FC<StorageTileCardProps> = ({
	...
	const label = storage.display || storage.name;
	...
	// Dynamic gradient based on storage name for visual distinction
	const avatarGradient = pickGradient(storage.name);
	...
	const cardContent = (
		<StyledFormTypeStorageBox
			...
			data-testId={formatToDataTestId(
				`importPageContent-connect-to-${storage.name}-img`,
			)}
		>
			{storage.link && !storage.disable && (
				<DocsLinkButton
					type="button"
					onClick={(e) => {
						e.stopPropagation();
						window.open(
							storage.link as string,
							"_blank",
							"noopener,noreferrer",
						);
					}}
			...
			<StyledInnerBox isStorage={true}>
				<Stack direction="row" width={"100%"} spacing={1}>
					{storage.icon ? (
						<StyledCardImage src={storage.icon} alt={initials} />
					) : (
						<StyledStorageAvatar gradientBg={avatarGradient}>
							{initials}
						</StyledStorageAvatar>
					)}
					{storage.disable && (
				...
				<TitleRow>
					<StyledCardStorageText ref={textRef}>
						{storage.display || storage.name}
					</StyledCardStorageText>
				</TitleRow>
