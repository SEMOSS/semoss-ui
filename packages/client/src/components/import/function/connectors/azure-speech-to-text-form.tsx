import {
	Form,
	FormActions,
	FormInput,
	FormSection,
	toast,
	useForm,
	z,
	zodResolver,
} from "@semoss/ui/next";
import { useRootStore } from "@/hooks";
import { useNavigate } from "@/hooks/useNavigate";
import {
	catalogNameSchema,
	createFunctionEngine,
} from "../shared/function-engine.utils";

const schema = z.object({
	NAME: catalogNameSchema,
	SPEECH_KEY: z.string().min(1, "Speech key is required"),
	SPEECH_REGION: z.string().min(1, "Speech region is required"),
});

type FormValues = z.infer<typeof schema>;

/** Add an "Azure Speech To Text" Function catalog entry. */
export const AzureSpeechToTextForm = () => {
	const navigate = useNavigate();
	const { configStore } = useRootStore();
	const form = useForm<FormValues>({
		resolver: zodResolver(schema),
		defaultValues: {
			NAME: "",
			SPEECH_KEY: "",
			SPEECH_REGION: "",
		},
	});

	const handleSubmit = async (values: FormValues) => {
		try {
			const engineId = await createFunctionEngine(
				values.NAME,
				{ FUNCTION_TYPE: "AZURE_SPEECH_TO_TEXT", ...values },
				undefined,
				configStore.store.insightID,
			);
			toast.success("Successfully added function database to catalog");
			navigate(`/function/${engineId}`);
		} catch (err) {
			console.error(err);
			toast.error(
				err instanceof Error ? err.message : "Something went wrong",
			);
		}
	};

	return (
		<Form
			form={form}
			onSubmit={handleSubmit}
			className="flex w-full flex-col gap-6"
		>
			<FormSection title="General" description="Name this catalog entry.">
				<FormInput
					name="NAME"
					label="Catalog Name"
					placeholder="Enter a name"
					disabled={form.formState.isSubmitting}
					data-testid="function-form-input-NAME"
				/>
			</FormSection>
			<FormSection
				title="Credentials"
				description="The Azure Speech resource key used to authenticate."
			>
				<FormInput
					name="SPEECH_KEY"
					label="Speech Key"
					disabled={form.formState.isSubmitting}
					data-testid="function-form-input-SPEECH_KEY"
				/>
			</FormSection>
			<FormSection
				title="Settings"
				description="Where the Azure Speech resource is located."
			>
				<FormInput
					name="SPEECH_REGION"
					label="Speech Region"
					disabled={form.formState.isSubmitting}
					data-testid="function-form-input-SPEECH_REGION"
				/>
			</FormSection>
			<FormActions
				isSubmitting={form.formState.isSubmitting}
				onCancel={() => navigate(-1)}
				submitLabel="Create"
			/>
		</Form>
	);
};
