...
const schema = z.object({
	...
	SMTP_SECURITY: z.string().min(1, "Connection security is required"),
...
/**
 * Add an "SMTP Email" Function catalog entry: sends email through an SMTP
 * mail server whose credentials live on this engine.
 */
export const SmtpEmailForm = () => {
		...
		>
			...
			>
				<FormInput
					...
					description="The address every email is sent from. Most relays reject a sender that does not match the authenticated account."
					disabled={form.formState.isSubmitting}
				...
				<FormInput
					...
					label="Password"
					type="password"
					description="Required whenever a username is set. Many providers need an app password rather than the account password."
					disabled={form.formState.isSubmitting}
					data-testid="function-form-input-SMTP_PASSWORD"
				/>
			</FormSection>
			<FormSection
				title="Settings"
			...
			>
				<FormInput
					name="FUNCTION_NAME"
					label="Function Name"
					description="Becomes the MCP tool name, so name it for what it does, ie send_email."
					disabled={form.formState.isSubmitting}
