...
/**
 * Project-scoped MCP toolbox editor — the `project=` twin of
...
export const ProjectMcpEditorPanel: React.FC<ProjectMcpEditorPanelProps> = ({
	...
	const readPixel = `GetAppAssets(filePath=["${config.path}"], project=["${project.project_id}"]);`;
	...
	const saveFile = async (data: MCPJsonData) => {
		try {
			...
			await insight.actions.run(
				`SaveAppAssets(project=["${project.project_id}"], filePath=["${config.path}"], content=["<encode>${JSON.stringify(
					data,
					null,
					2,
				)}</encode>"]);`,
