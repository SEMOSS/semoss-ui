...
import {
	FileHtmlEditorControl,
	type FileHtmlEditorControlValue,
} from "./file-html-editor-control";
import {
...
const FileHtmlEditorContent = lazy(() => import("./file-html-editor-content"));

export interface FileHtmlEditorParams {
...
const FileHtmlEditorPanel = ({
	config,
	id,
	rename,
	setValue,
}: WorkbenchPanelProps<FileHtmlEditorParams, FileHtmlEditorControlValue>) => {
	...
	// html files are edited as often as they are read, so the panel opens on the
	// source and the chrome's view select switches to the rendered page
	...
	/** Save the html source to its configured resource. */
	...
	useWorkbenchControl(id, FileHtmlEditorControl);
		...
		return (
			<div className="flex size-full items-center justify-center p-4">
				<Muted className="text-destructive" role="alert">
					{getFile.error?.message || "Failed to load html file"}
				</Muted>
			</div>
		);
	}
	...
	return (
		<div className="relative size-full overflow-hidden bg-background">
			{viewMode === "raw" ? (
				...
				>
					{/* edits render without saving first: the preview shows the
					    editor buffer, which is ahead of the file on disk */}
					<FileHtmlEditorContent html={content} path={config.path} />
				</Suspense>
			)}
			{access.refreshing ? (
				<WorkbenchAccessLoading
...
/** Scope-aware html editor blueprint shared by all workbenches. */
export const FILE_HTML_EDITOR_PANEL: WorkbenchPanelConfig<
	FileHtmlEditorParams,
	FileHtmlEditorControlValue
> = {
	name: "HTML",
	canRename: false,
	mount: "keepAlive",
