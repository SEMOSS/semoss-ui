...
import { Braces, ChevronsUpDown, Plus, Wand2 } from "lucide-react";
...
export const VariablesPanel = observer((): JSX.Element => {
	...
	useEffect(() => {
		...
		const cleanedEngines = getEngines.data.map((d) => ({
			engine_name: d.engine_name ? d.engine_name.replace(/_/g, " ") : "",
		...
		const newEngines = {
			...
			storages: cleanedEngines.filter((e) => e.engine_type === "STORAGE"),
			functions: cleanedEngines.filter(
				(e) => e.engine_type === "FUNCTION",
			),
			vectors: cleanedEngines.filter((e) => e.engine_type === "VECTOR"),
		};
	...
	return (
					...
					<Tooltip disableHoverableContent={false}>
						<TooltipTrigger asChild>
							{/* A disabled button emits no pointer events, so
							    the tooltip -- which is the only thing that says
							    why it is off -- hangs off a wrapper. */}
							<span className="inline-flex">
								<Button
									variant="ghost"
									size="icon-sm"
									aria-label="Suggest variable names"
									disabled={
										!workspace.agentModelEngine ||
										isProcessing
									}
						...
						<TooltipContent>
							{workspace.agentModelEngine
								...
								: "Select a model to suggest variable names"}
						</TooltipContent>
					</Tooltip>
					<Tooltip disableHoverableContent={false}>
						<TooltipTrigger asChild>
						...
						<TooltipContent>Create New Variable</TooltipContent>
					</Tooltip>
					<Tooltip disableHoverableContent={false}>
						<TooltipTrigger asChild>
							<Button
								...
								onClick={() => {
									const types = Object.keys(groupedVariables);
									const hasAnyExpanded = types.some(
										(type) => expandedItems[type] === true,
									);
									const newState: Record<string, boolean> =
										{};
									types.forEach((type) => {
										newState[type] = !hasAnyExpanded;
									});
		...
		>
			<div className="flex h-full flex-col overflow-auto bg-background">
				...
				>
					{Object.entries(groupedVariables).map(([type, vars]) => {
						if (expandedItems[type] === undefined) return null;
						return (
							<AccordionItem
								key={type}
								value={type}
							...
							>
								<AccordionTrigger
									className="min-h-8 border-border border-t bg-muted/30 px-2 py-1 text-foreground hover:bg-accent hover:no-underline"
									onClick={() => {
										setExpandedItems((prev) => ({
											...prev,
											[type]: !prev[type],
			...
			<Dialog
				...
				onOpenChange={(open) => !open && setIsRenameModalOpen(false)}
			>
				<DialogContent
					aria-describedby={undefined}
					className="max-h-[80vh] w-[calc(100vw-2rem)] max-w-2xl overflow-auto"
				>
					<DialogHeader>
					...
					<p className="text-muted-foreground text-sm">
						Review and select the variable name changes you'd like
						to apply. All changes are selected by default.
					</p>
