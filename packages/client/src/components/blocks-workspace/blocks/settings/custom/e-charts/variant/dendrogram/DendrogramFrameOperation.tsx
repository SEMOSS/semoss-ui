...
export const DendrogramFrameOperation = observer(
    <D extends BlockDef = BlockDef>({ id }) => {
        ...
        const [frameField, setFrameField] = useState({
            dimensions: [],
            facet: '',
        });
        ...
        function updateFields(fieldName, fieldValue) {
            if (fieldName === 'facet') {
                ...
                let facetExistsInDimensions = dimensionsList.some(
        ...
        useEffect(() => {
            ...
            if (frameField['facet'] != '') {
                let name = columnsSelector.find(
                    (column) => column.selector === frameField['facet'],
                );
