  ...
  <D extends BlockDef = BlockDef>({
    ...
    useEffect(() => {
        ...
        if (
          ...
          ["xAxis", "yAxis"].forEach((item) => {
            dataTypeList[item] = columnsSelector
              .filter((col) => parsedOption[item].pixelname.includes(col.name))
      ...
      if (data.variation === "echart-dendrogram-chart") {
        ...
        const tempStoredColumns = chart.map((item) => {
          return {
            name: item.name,
            label: item.label,
            values: parsedOption["_state"][item.label].map((val) => val.name),
        ...
        if (
          ...
          const tempStoredColumns = chart.map((item) => {
            return {
              name: item.name,
              label: item.label,
              values: parsedOption["_state"]["fields"][item.label].filter(
                (field) => field && Object.hasOwn(field, "name")
              ).length
                ? parsedOption["_state"]["fields"][item.label]?.map(
                    (item) => item?.name || ""
                  )
                : [],
              selectors: parsedOption["_state"]["fields"][item.label].filter(
                (field) => field && Object.hasOwn(field, "selector")
              ).length
                ? parsedOption["_state"]["fields"][item.label]?.map(
                    (item) => item?.selector || ""
                  )
        ...
        if (
          ...
          const tempStoredColumns = chart.map((item) => {
            return {
              name: item.name,
              label: item.label,
              values: parsedOption["_state"]["fields"][item.label].some(
                (valItem) => valItem && Object.hasOwn(valItem, "name")
              )
                ? parsedOption["_state"]["fields"][item.label].map(
                    (val) => val.name
                  )
                : [],
              selectors: parsedOption["_state"]["fields"][item.label].some(
                (valItem) => valItem && Object.hasOwn(valItem, "selector")
              )
                ? parsedOption["_state"]["fields"][item.label].map(
                    (selector) => selector.selector
                  )
      ...
      if (data.variation === "echart-world-map-chart") {
        ...
        if (
          ...
          [
            ...
            dataTypeList[item] = columnsSelector
              .filter(
                (col) => parsedJson["_state"]["fields"][item] === col.name
              )
              .map((col) => col.dataType);
            selectorList.push(
              columnsSelector.find(
                (col) => col.name === parsedJson["_state"]["fields"][item]
              )?.selector || ""
            );
          });
          const tempStoredColumns = chart.map((item, index) => {
            return {
              name: item.name,
              label: item.label,
              values: Object.hasOwn(parsedJson["_state"]["fields"], item.label)
                ? Array.isArray(parsedJson["_state"]["fields"][item.label])
      ...
      if (data.variation === "echart-scatter-plots") {
        ...
        if (
          ...
          const dataTypeList = {};
          const selectorList = [];
          ["label", "XAxis", "YAxis", "size", "color", "tooltip"].forEach(
            (item) => {
        ...
        if (
          ...
          ["XAxis", "YAxis", "category", "tooltip"].forEach((item) => {
            dataTypeList[item] =
              columnsSelector
                .filter((col) =>
                  parsedJson["_state"]["fields"][item].includes(col.name)
                )
                .map((col) => col.dataType) || [];
            selectorList.push(
              columnsSelector.find((col) =>
          ...
          [
            ...
            dataTypeList[item] =
              columnsSelector
                ...
                .map((col) => col.dataType) || [];
            console.log(
              parsedJson["customSettings"]["columnDetails"][item]?.["name"],
              columnsSelector.find(
                (col) =>
                  parsedJson["customSettings"]["columnDetails"][item]?.[
                    "name"
    ...
    const formattedColumns = (columnsValue: any[], variation: any) => {
        ...
        if (columns[firstColumn?.label] && columns[secondColumn?.label]) {
          ...
          for (let i = 0; i < columns[secondColumn?.label]?.length; i++) {
            tempVal["series"][i] = {
              ...
              barWidth: 5,
              ["itemStyle"]: {
                ["color"]: tempVal["color"][i] ?? COLOUR_PALATTE_DATA[i],
              },
            };
          }
          tempVal = {
      ...
      if (variation == "echart-gantt-chart") {
        ...
        for (let i = 0; i < columnsValue.length; i++) {
          if (columnsValue[i]?.values.length == 1) {
            if (
              formattedArray.some(
                (item) => item.name === columnsValue[i]?.values?.[0]
              )
            )
              continue;
            formattedArray.push({
            ...
            for (let j = 0; j < columnsValue[i]?.values.length; j++) {
              if (
                formattedArray.some(
                  (item) => item.name === columnsValue[i]?.values?.[j]
                )
              )
                continue;
              formattedArray.push({
        ...
        if (columnsDrop[6]?.values.length > 0) {
          ...
          columnsObject["tooltip"] = Array.isArray(columnsDrop[6]?.values)
      ...
      const checkAggregate = (functionName) =>
        ({ NUMBER: "Average", STRING: "Count" }[functionName] || functionName);
        ...
        {accordionSection.map((item, index) => (
          <Accordion
            ...
            onChange={(e) => {
              ...
              const indexToUpdate = accordionSectionToUp.findIndex(
                (accordItem) => Object.hasOwn(accordItem, accordionList[index])
              );
    ...
    return (
      <>
        <DragDropContext onDragEnd={handleDragEnd}>
                                  ...
                                  onChange={(e) => {
                                    setDroppedColumns((prev) => {
                                      ...
                                      if (e.target.checked) {
                                        ...
                                        updated[addedColumnName] = {
                                          values: [
                                            ...updated[addedColumnName].values,
                                            col.name,
                                          ],
                                          dataType: [
                                            ...updated[addedColumnName]
                                        ...
                                        if (updated[addedColumnName]) {
                                          const index = updated[
                                            addedColumnName
                                          ]["values"].indexOf(col.name);
                                          updated[addedColumnName] = updated[
                                            addedColumnName
          ...
          <StyledDropDownSection>{renderAccordion}</StyledDropDownSection>
        </DragDropContext>
      </>
    );
  }
