...
const _StyledTableTitleProjectCountContainer = styled("div")({
  display: "flex",
  height: "56px",
...
export const TeamProjectsTable = (props: ProjectsTableProps) => {
  ...
  const [selectedNonCredentialedProjects, setSelectedNonCredentialedProjects] =
    useState([]);
  ...
  const deleteProject = async (project) => {
    try {
      let response:
        ...
        | null = null;
      response = await deleteProjectPermission(groupId, groupType, project);
  ...
  return (
    <StyledProjectContent>
              ...
              <StyledDeleteSelectedContainer>
                {selectedProjects.length > 0 && (
                  <Button
                    variant={"outlined"}
                    color="error"
                    onClick={() => setDeleteProjectsModal(true)}
                  >
                    Delete Selected
                  </Button>
                )}
            ...
            <StyledProjectTable>
              <Table.Head>
                <Table.Row>
                  ...
                  <Table.Cell size="small">Added Date</Table.Cell>
              ...
              <Table.Body>
                {projects &&
                  projects.map((x, i) => {
                    ...
                    if (project) {
                      isSelected = selectedProjects.some((value) => {
                    ...
                    if (project) {
                      return (
                        <Table.Row key={project.projectid + i}>
                            ...
                            <StyledCheckbox
                              ...
                              onChange={() => {
                                if (isSelected) {
                                  ...
                                  setSelectedProojects(selProjects);
                                } else {
                                  setSelectedProojects([
                                    ...selectedProjects,
                                    project,
                          ...
                          >
                            <NameIDWrapper>
                              <Stack>{project.project_name}</Stack>
                              <Stack>
                          ...
                          <Table.Cell size="medium">
                            <RadioGroup
                              row
                              defaultValue={project.permission}
                              onChange={(e) => {
                                console.log(
                            ...
                            >
                              <RadioGroup.Item value="1" label="Author" />
                              <RadioGroup.Item value="2" label="Editor" />
                              <RadioGroup.Item value="3" label="Read-Only" />
                            </RadioGroup>
                          </Table.Cell>
                          <Table.Cell size="medium">
      ...
      <StyledModal open={addProjectModal} maxWidth="lg">
                            ...
                            onClick={() => {
                              const filtered =
                                selectedNonCredentialedProjects.filter(
                                  (val) => val.project_id !== project.project_id
                                );
                              setSelectedNonCredentialedProjects(filtered);
                            }}
                          >
                            <ClearRounded />
                          </IconButton>
                  ...
                  <StyledCard>
                    <Card.Header
                      ...
                      subheader={
                        ...
                        >
                          Ability to edit the model connection details, set the
                          model as discoverable, provision other authors, and
                          all editor abilities.
                        </Box>
                      }
                      action={<RadioGroup.Item value="Author" label="" />}
                    />
                  </StyledCard>
                  <StyledCard>
                    <Card.Header
                      ...
                      subheader={
                        ...
                        >
                          Ability to edit the model details, provision other
                          users as editors and read only users, and all read
                          only abilities.
                        </Box>
                      }
                      action={<RadioGroup.Item value="Editor" label="" />}
                    />
                  </StyledCard>
                  <StyledCard>
                    <Card.Header
                      ...
                      subheader={
                        ...
                        >
                          Ability to view model details and usage instructions
                        </Box>
                      }
                      action={<RadioGroup.Item value="Read-Only" label="" />}
                    />
                  </StyledCard>
                </Stack>
              </RadioGroup>
        ...
        <Modal.Actions>
          ...
          <Button
            variant={"contained"}
            disabled={
              !addProjectRole || selectedNonCredentialedProjects.length < 1
            }
            onClick={() => {
          ...
          <Modal.ContentText>
            {projectToDelete && (
              <Typography variant="body1">
                This will remove <b>{projectToDelete.project_name}</b>
              </Typography>
            )}
          </Modal.ContentText>
        </Modal.Content>
        <Modal.Actions>
          <Button variant="text" onClick={() => setDeleteProjectModal(false)}>
            Close
          </Button>
          <Button
            color="error"
      ...
      <Modal open={deleteProjectsModal}>
        ...
        <Modal.Actions>
          <Button variant="text" onClick={() => setDeleteProjectsModal(false)}>
            Close
          </Button>
          <Button
            variant={"contained"}
