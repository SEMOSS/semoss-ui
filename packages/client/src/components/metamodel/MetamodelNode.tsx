...
type MetamodelNodeProps = NodeProps<{
    ...
    properties: {
        ...
        physicalType?: string; // column phyisical type
        /** Specific format for column type */
...
const _MetamodelNode = (props: MetamodelNodeProps) => {
    ...
    const { selectedNodeId, onSelectNodeId, isInteractive, updateData } =
        useMetamodel();
    const [openEditColumnModal, setOpenEditColumnModal] = useState(false); // open edit column modal
    const [editTableMode, setEditTableMode] = useState(false); // when edit table mode is on, transforms styling of table and allows user to edit table
    ...
    const StyledTableFooterRow = styled('div')(({ theme }) => ({
        display: 'flex',
        height: '44px',
        width: editTableMode ? '373px' : `${metamodelCardWidth}px`,
        justifyContent: 'center',
        alignItems: 'center',
