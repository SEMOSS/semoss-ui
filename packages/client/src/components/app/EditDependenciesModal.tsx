...
export const EditDependenciesModal = (props: EditDependenciesModalProps) => {
  ...
  const modelDependencies = (dependencies: engine[]): modelledDependency[] => {
  ...
  return (
    <Modal open={isOpen} fullWidth onClose={() => onClose(false)}>
      <StyledModalHeading>
        <StyledHeader variant="h6">Add and Edit Dependencies</StyledHeader>
        ...
        {selectedDeps.map((dep: modelledDependency, idx: number) => {
          return (
            <StyledDependencyListItem key={`${dep.id}-${idx}`}>
              ...
              <IconButton onClick={() => handleRemoveDependency(dep.id)}>
                <Close />
              </IconButton>
