  ...
  const handleRequestAllAccess = async () => {
    ...
    try {
      ...
      const dependenciesToRequest = dependencies.filter(
        (dep) =>
          dep.userPermission !== requestedRole && !requestedDeps.has(dep.id)
      );
      ...
      <Modal open={open} maxWidth={"md"} onClose={onClose}>
        ...
        {permission !== "discoverable" ? (
          <StyledDivider>
            ...
            >
              <Tab label="App Permissions" aria-controls="tab-panel-0" />
              <Tab label="Dependency Permissions" aria-controls="tab-panel-1" />
            </Tabs>
          </StyledDivider>
        ) : null}
                  ...
                  <StyledContentBox direction="column" gap={1}>
                    <StyledContentCard direction="row" gap={1}>
                      ...
                      <StyledRoleInfo>
                        ...
                        <span>{permissionDescriptions.author}</span>
                      </StyledRoleInfo>
                      <RadioGroup
                        label=""
                        value={field.value}
                      ...
                      >
                        <RadioGroup.Item value="OWNER" label="" />
                      </RadioGroup>
                    </StyledContentCard>
                    ...
                    <StyledContentCard direction="row" gap={1}>
                      ...
                      <StyledRoleInfo>
                        ...
                        <span>{permissionDescriptions.editor}</span>
                      </StyledRoleInfo>
                      <RadioGroup
                        label=""
                        value={field.value}
                        onChange={(val) => field.onChange(val)}
                      >
                        <RadioGroup.Item value="EDIT" label="" />
                      </RadioGroup>
                    </StyledContentCard>
                    ...
                    <StyledContentCard direction="row" gap={1}>
                      ...
                      <StyledRoleInfo>
                        ...
                        <span>{permissionDescriptions.readonly}</span>
                      </StyledRoleInfo>
                      <RadioGroup
                        label=""
                        value={field.value}
                        onChange={(val) => field.onChange(val)}
                      >
                        <RadioGroup.Item value="READ_ONLY" label="" />
                      </RadioGroup>
                    </StyledContentCard>
                  </StyledContentBox>
                );
        ...
        <TabPanel value={tabValue} index={1}>
          <Modal.Content>
            <ModelSubHeading variant={"body2"}>
              The app will not work for you without having at least read-only
              access to the following dependencies. Click request access to be
            ...
            <StyledButtonBox>
              <StyledButton
                ...
                disabled={
                  isAllRequested ||
                  isRequestAllLoading ||
                  dependencies.some((dep) => dep.access_permission)
                }
              >
                {isRequestAllLoading ? "Requesting..." : "Request All Access"}
              </StyledButton>
            </StyledButtonBox>
            <CardContentOuterBox>
              <Stack spacing={2}>
                {dependencies.map((dep) => (
                  <CardSubContentOuterBox key={dep.id}>
                    ...
                    <CardContentInnerBox>
                      <Container>
                        <EngineImage src={OPEN_AI} alt={dep.name} />
                        <Box>
                          ...
                          <PermissionWrapper>
                            {dep.userPermission === "OWNER" ? (
                              <StyledOwnerIcon fontSize="small" />
                            ) : dep.userPermission === "READ_ONLY" ? (
                              <StyledReadonlyIcon fontSize="small" />
                            ) : dep.userPermission === "EDIT" ? (
                        ...
                        <ActionsWrapper direction="row" spacing={1}>
                          <Stack direction="row" spacing={1}>
                            ...
                            <PublicChip label={toCapitalized(dep.type)} />
                          </Stack>
                          ...
                          <Box>
                            {dep.access_permission ? (
                              ...
                              <ActionButton
                                ...
                                onClick={() =>
                                  handleSingleDependencyRequest(dep.id)
                                }
                              />
                            ) : (
                              <ActionButton
