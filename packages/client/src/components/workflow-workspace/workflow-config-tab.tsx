...
import type { WorkflowConfigEntry } from "@/pages/workflow/workflow.types";

interface WorkflowConfigTabProps {
	config: WorkflowConfigEntry[];
	onChange: (config: WorkflowConfigEntry[]) => void;
}
...
export function WorkflowConfigTab({
	...
	const update = (i: number, patch: Partial<WorkflowConfigEntry>) =>
	...
	return (
		<div className="flex flex-col gap-4 p-4">
			<div>
				<h3 className="font-semibold text-sm">Workflow Config</h3>
				<p className="mt-0.5 text-muted-foreground text-xs">
					Key-value pairs stored in this workflow's SMSS. Reference
					them in any node field as{" "}
