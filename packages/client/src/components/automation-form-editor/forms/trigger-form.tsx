...
export function TriggerForm({
	...
	const webhookUrl = `${window.location.origin}${Env.MODULE}/api/automation/${appId}/trigger`;
	...
	const activateSchedule = async () => {
		...
		try {
			...
			const recipe = `TriggerAutomation(project=["${appId}"])`;
	...
	const generateSecret = async () => {
		if (!onGenerateWebhookSecret) return;
	...
	const activatePoll = async (pollType: "storage-poll" | "db-poll") => {
		...
		try {
			...
			const recipe = `CheckAutomationPollTrigger(project=["${appId}"], type=["${pollType}"])`;
			...
			{config.mode === "manual" && (
				<div className="rounded-md border border-border bg-muted/30 p-3">
					...
					<p className="mt-1.5 text-[10px] text-muted-foreground">
						...
						insight to trigger this automation on demand.
					</p>
				</div>
			)}
				...
				<>
					...
					<div className="flex flex-col gap-2 rounded-md border border-border bg-muted/30 p-3">
						...
						<Button
							...
							disabled={
								generatingSecret || !onGenerateWebhookSecret
							}
						>
							{generatingSecret ? (
			...
			{config.mode === "storage-poll" && (
				<>
					...
					<Field>
						...
						/>
						<p className="mt-1 text-[10px] text-muted-foreground">
							Automation fires when the file list in this path
			...
			{config.mode === "db-poll" && (
				<>
					...
					<Field>
						...
						/>
						<p className="mt-1 text-[10px] text-muted-foreground">
							The automation fires when this query's result
							changes between polls. Good patterns:{" "}
