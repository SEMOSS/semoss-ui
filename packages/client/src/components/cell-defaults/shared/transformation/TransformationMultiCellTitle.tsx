...
export const TransformationMultiCellTitle: CellComponent<
    TransformationMultiCellDef
> = (props) => {
    const { cell } = props;
    ...
    const changeTransformationType = (newTransformationWidget: string) => {
        ...
        let config: NewCellAction['payload']['config'] = {
            ...
            parameters: {
                ...
                fromTargetCell: { ...cell.parameters.fromTargetCell },
                toTargetCell: { ...cell.parameters.toTargetCell },
            },
        };
    ...
    useEffect(() => {
        if (
            ...
            if (!cell.query.list.includes(cell.parameters.fromTargetCell.id)) {
                state.dispatch({
            ...
            if (!cell.query.list.includes(cell.parameters.toTargetCell.id)) {
                state.dispatch({
                    message: ActionMessages.UPDATE_CELL,
                    payload: {
                        queryId: cell.query.id,
            ...
            if (
                !cell.query.cells[cell.parameters.fromTargetCell.id] ||
                !isValidFrameTypeForTransformation(
                    cell.query.cells[cell.parameters.fromTargetCell.id]
                        .parameters.frameType as string,
                )
            ) {
                state.dispatch({
                    message: ActionMessages.UPDATE_CELL,
            ...
            if (
                !cell.query.cells[cell.parameters.toTargetCell.id] ||
                !isValidFrameTypeForTransformation(
                    cell.query.cells[cell.parameters.toTargetCell.id].parameters
                        .frameType as string,
                )
            ) {
                state.dispatch({
        ...
        cell.parameters.fromTargetCell,
        cell.parameters.toTargetCell,
        availableFrameCells,
    ]);

    const [menuType, setMenuType] = useState<
        'routine' | 'fromFrame' | 'toFrame'
    >(null);
        ...
        <>
            <StyledButton
                ...
                onClick={(event: React.MouseEvent<HTMLElement>) => {
                    ...
                    setMenuType('fromFrame');
                    setAnchorEl(event.currentTarget);
                }}
                startIcon={<FilterFrames />}
                endIcon={<KeyboardArrowDown />}
                title="Select 'From' Frame Variable"
            >
            ...
            <StyledButton
                ...
                startIcon={<FilterFrames />}
                endIcon={<KeyboardArrowDown />}
                title="Select 'To' Frame Variable"
            >
                <StyledButtonLabel width={14}>
