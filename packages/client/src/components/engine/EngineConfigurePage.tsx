...
const StyledChip = styled(Chip)<{ isSelected: boolean }>(
  ({ isSelected, theme }) => ({
    backgroundColor: isSelected ? theme.palette.primary.selected : "default",
    color: isSelected ? theme.palette.primary.dark : "default",
    cursor: "pointer",
  })
);
...
export const EngineConfigurePage = () => {
  ...
  const handleOnSelect = (path: string) => {
    if (path.includes(".")) {
      ...
      setFileList((prevList) => {
        if (!prevList.includes(normalizedPath)) {
          return [...prevList, normalizedPath];
        }
        return prevList;
      });
  ...
  const handleFileClose = (filePathToRemove: string) => {
    setFileList((prevList) => {
      const updatedList = prevList.filter((file) => file !== filePathToRemove);
  ...
  useEffect(() => {
    if (Array.isArray(assetData)) {
      ...
      const fileNames = assetData
        .filter((f) => f.type !== "directory")
        .map((f) => f.path.replace(/\\/g, "/")); // Normalize paths

      setFileList(fileNames);

      // Optionally select the first file
      const initialFile = assetData.find(
        (f) => f.type !== "directory" && f.path
      );
      if (initialFile) {
    ...
    <StyledLayout>
      <Stack direction="row">
        ...
        <StyledDiv>
            ...
            <DndContext
              ...
              onDragEnd={({ active, over }) => {
                if (active.id !== over?.id) {
                  const oldIndex = fileList.indexOf(active.id as string);
                  const newIndex = fileList.indexOf(over?.id as string);
                  const reordered = arrayMove(fileList, oldIndex, newIndex);
                  ...
                  {fileList.map((filePath) => {
                    ...
                    return (
                      <SortableChip
                        ...
                        onDelete={() => handleFileClose(filePath)}
                      />
                    );
                  })}
                </Stack>
          ...
          {selectedPath ? (
            <EngineFileEditor engineId={active.id} filePath={selectedPath} />
          ) : (
            <StyledEmptyDiv>
              <Typography variant="body1" color="secondary">
                Upload or Create file to view its contents
              </Typography>
            </StyledEmptyDiv>
          )}
        </StyledDiv>
      ...
      {showAddFileModal && (
        <Modal open onClose={() => setShowAddFileModal(false)} fullWidth>
      ...
      {showCreateFileModal && (
        <Modal open onClose={() => setShowCreateFileModal(false)} fullWidth>
      ...
      {showDeleteModal && (
        <Modal open={true} onClose={() => setShowDeleteModal(false)} fullWidth>
