 ...
 * system database.
 */
export const AdminQueryWorkspace: React.FC = observer(() => {
	...
	const getDatabaseStructure = usePixel<unknown[]>(
		engine.engine_id
			? `AdminGetSystemDatabaseSchema(database=[${JSON.stringify(engine.engine_id)}]);`
			: "",
	);
	...
	const onRun = async (query: string, panelId: string) => {
		...
		try {
			...
			const pixel = `AdminSqlQuery(database=["${engine.engine_id}"], query=["<encode>${q}</encode>"], commit=[true]);`;
