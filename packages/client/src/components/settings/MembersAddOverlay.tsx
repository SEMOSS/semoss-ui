...
export const MembersAddOverlay = (props: MembersAddOverlayProps) => {
  ...
  useEffect(() => {
    ...
    if (user) {
      setSelectedRole(
        permissionPriorityMapper(user?.permission)?.permission as SETTINGS_ROLE
      );
      setRestriction(
        user?.usage_restriction !== undefined ? user?.usage_restriction : "null"
      );
  ...
  const addMembers = async () => {
    ...
    try {
      ...
      if (type === "MODEL") {
        requests = selectedMembers.map((m) => {
          return {
            ...
            type: m.type,
            username: m.username,
            usageRestriction: restriction === "null" ? null : restriction,
      ...
      if (
        ...
        response = await addEngineUserPermissions(adminMode, id, requests);
      } else if (type === "APP") {
        response = await addProjectUserPermissions(adminMode, id, requests);
      }
  ...
  return (
    <Modal open={open} maxWidth="lg">
                ...
                <StyledCardHeader
                  ...
                  action={
                    <RadioGroup.Item
                      ...
                      disabled={
                        !adminMode &&
                        permissionPriorityMapper(userPermission)?.priority > 1
                      }
                    />
                  }
                />
                ...
                <Card.Header
                  ...
                  action={
                    <RadioGroup.Item
                      ...
                      disabled={
                        !adminMode &&
                        permissionPriorityMapper(userPermission)?.priority > 2
                      }
                    />
                  }
                />
                ...
                <Card.Header
                  ...
                  action={
                    <RadioGroup.Item
                      ...
                      disabled={
                        !adminMode &&
                        permissionPriorityMapper(userPermission)?.priority > 3
                      }
                    />
                  }
                />
        ...
        {type === "MODEL" && (
          <>
            ...
            <Stack direction={"column"} gap={1}>
              ...
              >
                {Object.entries(usageRestritctionTypes).map((option, i) => {
                  return (
                    <Select.Item value={option[0]} key={i}>
                      {option[1]}
                    </Select.Item>
                  );
                })}
                ...
                <Stack direction={"row"} gap={1}>
                  ...
                  <Select label="Unit" value={unitTypes[0]}>
                    {unitTypes.map((option, i) => {
                      return (
                        <Select.Item value={option} key={i}>
                          {option}
                        </Select.Item>
                      );
                    })}
              ...
              {restriction !== "null" && (
                ...
                >
                  {Object.entries(frequencyTypes).map((option, i) => {
                    return (
                      <Select.Item value={option[0]} key={i}>
                        {option[1]}
                      </Select.Item>
      ...
      <Modal.Actions>
        <Button variant="outlined" onClick={() => closeOverlay(type, false)}>
          Cancel
        </Button>
        {user === null && (
          <Button
