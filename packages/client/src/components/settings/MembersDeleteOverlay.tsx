...
export const MembersDeleteOverlay = (props: MembersDeleteOverlayProps) => {
  const { type, id, members = [], open = false, onClose = () => null } = props;
  ...
  const deleteSelectedMembers = async (
    ...
    try {
      ...
      if (
        ...
        response = await removeEngineUserPermissions(adminMode, id, requests);
      } else if (type === "APP") {
        response = await removeProjectUserPermissions(adminMode, id, requests);
      }
