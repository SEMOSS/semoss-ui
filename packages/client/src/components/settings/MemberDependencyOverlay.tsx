...
export const MemberDependencyOverlay = (
    ...
    (async () => {
      ...
      for (const dep of allDependencies) {
        try {
          ...
          const userObj = users.find((u) => u.id === selectedMember.id);
          if (userObj) {
  ...
  const nearBottom = (target: EventTarget | null) => {
    if (
      !target ||
      typeof (target as unknown as { scrollHeight: number }).scrollHeight !==
        "number"
    )
      return false;
            ...
            {checkingAccess ? (
              ...
              <Box>
                <Box>
                  <Typography
                    variant="subtitle1"
                    sx={{ mb: 2, fontWeight: 500, color: "success.main" }}
                  >
                    Has Access
                  </Typography>
                  <Box
                  ...
                  >
                    {allDependencies
                      .filter((dep) => userDependencies.includes(dep.engine_id))
                      .map((dep) => (
                        <Box
                          key={dep.engine_id}
                          sx={{
                        ...
                        >
                          ...
                          <Box sx={{ flex: "0 0 60%", maxWidth: "60%" }}>
                            <RadioGroup
                              row
                              value={
                                pendingPermissions[dep.engine_id] ||
                                (enginePermissions[dep.engine_id] === "OWNER"
                                  ? "Author"
                                  : enginePermissions[dep.engine_id] === "EDIT"
                                  ? "Editor"
                                  : enginePermissions[dep.engine_id] ===
                                    "READ_ONLY"
                                  ? "Read-Only"
                                  : "")
                              }
                ...
                <Box sx={{ mt: 2 }}>
                  ...
                  >
                    {allDependencies
                      .filter((dep) => userNoAccess.includes(dep.engine_id))
                      .map((dep) => (
                        <Box
                          key={dep.engine_id}
                          sx={{
                        ...
                        >
                          ...
                          <Box sx={{ flex: "0 0 60%", maxWidth: "60%" }}>
                            <RadioGroup
                              row
                              value={pendingPermissions[dep.engine_id] || ""}
                              sx={{ ml: 2, gap: 2 }}
                              onChange={(e) => {
                                setPendingPermissions((prev) => ({
                                  ...prev,
          ...
          onClick={async () => {
            ...
            for (const [engineId, permission] of Object.entries(
              ...
              if (response?.data?.success) {
                ...
                if (!userDependencies.includes(engineId)) {
                  setUserDependencies((prev) => [...prev, engineId]);
                ...
                const engineName = depObj?.engine_name || engineId;
                failedCalls.push(`${engineName} (${permission})`);
              }
            }
            if (success) {
              notification.add({
