  ...
  return (
    <StyledFileContent>
          ...
          <Table.Head>
            <Table.Cell size="small">
              <Checkbox
                ...
                onChange={() => {
                  if (selectedFiles.length !== verifiedFiles.length) {
            ...
            <Table.Cell size="small">
              <Table.Sort
                active={true} // sort icon is always visible
                direction={orderBy === headCell[2].id ? order : "asc"}
          ...
          <Table.Body>
            {getFileDetails.status === "LOADING" ? (
              ...
              verifiedFiles.map((_x, i) => {
                if (
                  i >= filePage * NUM_RESULTS_PER_PAGE - NUM_RESULTS_PER_PAGE &&
                  i < filePage * NUM_RESULTS_PER_PAGE
                ) {
                  const file = verifiedFiles[i];
                  const isSelected = file
                    ? selectedFiles.some(
                        (value) => value.fileName === file.fileName
                      )
                    : false;
                  ...
                  if (file) {
                    return (
                      <Table.Row key={file.fileName}>
                        ...
                        <Table.Cell size="medium" component="td" scope="row">
                          {file.fileName}
                        </Table.Cell>
                        <Table.Cell size="medium" component="td" scope="row">
                          {file.lastModified}
      ...
      <Modal open={deleteFileModal} maxWidth="md">
        ...
        <Modal.Actions>
          <Button variant="text" onClick={() => setDeleteFileModal(false)}>
            Close
          </Button>
          <Button
            color="error"
      ...
      <Modal open={deleteFilesModal}>
        ...
        <Modal.Actions>
          <Button variant="text" onClick={() => setDeleteFilesModal(false)}>
            Close
          </Button>
          <Button
            variant={"contained"}
