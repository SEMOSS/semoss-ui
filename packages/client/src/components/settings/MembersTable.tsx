...
export const MembersTable = (props: MembersTableProps) => {
  ...
  /** Delete Member */
  const [deleteMembersModal, setDeleteMembersModal] = useState<boolean>(false);
    ...
    if (adminMode) {
      ...
      setUserPermission(
        permissionPriorityMapper(
          userData.permission === "OWNER" ? "Author" : userData.permission
        )?.permission as SETTINGS_ROLE
      );
    }
  ...
  const readOnlyRestricted = (member) => {
    ...
    return (
      (type === "DATABASE" || type === "APP") && member.name === userData.name
    );
  };
  ...
  const openDeleteMembersModal = (
    ...
    const authorsToDelete = selectedMembers.filter(
      (m) => permissionPriorityMapper(m.permission)?.permission === "Author"
    );
    if (allAuthors.length > 0 && authorsToDelete.length === allAuthors.length) {
      notification.add({
        color: "error",
  ...
  const sortedMembers = useMemo(() => {
    ...
    const getPermissionOrder = (permission: string): number => {
      ...
      return (
        permissionOrder[permissionPriorityMapper(permission)?.permission] || 0
      );
    };
  ...
  const isLastAuthor = (user) => {
    ...
    return (
      permissionPriorityMapper(user.permission)?.permission === "Author" &&
      authors.length === 1 &&
                ...
                <Table.Body>
                  {[...Array(rowsPerPage)].map((item) => (
                    <Table.Row key={item}>
                      ...
                      <Table.Cell size="medium">
                        <Skeleton variant="text" width={240} height={35} />
                      </Table.Cell>
                      <Table.Cell size="medium">
                        <Skeleton variant="text" width={160} height={35} />
                      </Table.Cell>
                    ...
                    <Table.Head>
                      <Table.Row>
                        <Table.Cell size="small" padding="checkbox">
                          <Checkbox
                            disabled={userPermission === "Read-Only"}
                            checked={
                            ...
                            onChange={() => {
                              if (
                                ...
                                setSelectedMembers(renderedMembers);
                              } else {
                                setSelectedMembers([]);
                              }
                            }}
                        ...
                        <Table.Cell size="small">Permission Date</Table.Cell>
                        {type === "MODEL" && (
                          <>
                            <Table.Cell size="small">
                              Model Limit Type
                              ...
                              <Table.Cell size="medium">
                                <StyledRadioGroup
                                  ...
                                  onChange={(e) => {
                                    updateSelectedUsers(
                                      [user],
                                      permissionPriorityMapper(e.target.value)
                                        ?.permission
                                    );
                                  }}
                                >
                                  <RadioGroup.Item
                                    ...
                                    disabled={
                                      (!configStore.isEngineOperationAvailable(
                                        ...
                                        permissionPriorityMapper(userPermission)
                                          .priority > 1) &&
                                      !adminMode
                                    }
                                  />
