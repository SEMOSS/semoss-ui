...
const META_UI_KEYS: MetaKeyDoc[] = [
	{
		key: "loadingMessage",
		type: "string",
		desc: 'Message shown while the tool runs, e.g. "Searching the database…". Falls back to default messages if omitted.',
	},
	{
		key: "displayLocation",
...
const METADATA_EXAMPLE = `{
  ...
  "SMSS_MCP_UI": {
    "loadingMessage": "Searching the database…",
 ...
 * persist metadata from Python/Java) in a modal so the guidance isn't always
 * taking up space inline.
 */
export const MetadataHelpDialog: React.FC<MetadataHelpDialogProps> = ({
	compact = false,
	className,
}) => (
	<Dialog>
...
export const MCPJsonEditor: React.FC<MCPJsonEditorProps> = ({ dataMap }) => {
	...
	const headerText = useMemo(() => {
		...
		// Friendly names for the known MCP driver files; fall back to the raw
		// upper-cased file name for anything else.
		...
		return label
			? `${label} - MCP JSON Editor`
			: `${fileName.toUpperCase()} JSON Editor`;
	}, [path]);
