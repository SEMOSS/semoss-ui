...
(() => {
	...
	window.addEventListener("message", (event) => {
		...
		if (event.data?.type === "PING_EXTENSION") {
			console.log(
				"[MAIN-BRIDGE] 🏓 Received PING from portal, relaying to isolated",
			);
		...
		if (event.data?.type === "EXECUTE_PLAYWRIGHT_SCRIPT") {
			console.log(
				"[MAIN-BRIDGE] 📤 Received EXECUTE from portal, relaying to isolated",
			);
	...
	window.addEventListener("message", (event) => {
		...
		if (event.data?.type === "EXTENSION_READY_BRIDGE") {
			console.log(
				"[MAIN-BRIDGE] ✅ Received READY from isolated, sending to portal",
			);
