...
const PanelApp: React.FC = () => {
	...
	const [pendingPlaygroundCommand, setPendingPlaygroundCommand] = useState<
		string | null
	>(null);
		...
		const messageListener = (
			...
			if (message.type === "PLAYGROUND_CHAT_RESPONSE") {
				...
				const scriptPatterns =
					/play\s+(some\s+)?script|run\s+(a\s+)?script|execute\s+(a\s+)?script/i;
				if (scriptPatterns.test(aiResponseText)) {
					setMode("script");
					setActionHistory([
						`🎮 Switched to Script Mode from Playground AI response`,
					]);
					return;
				}
				...
				if (mode === "llm" && aiResponseText.trim()) {
					...
					if (autoExecutePlayground && !isRunning) {
						setActionHistory([
							`🎮 Auto-executing from Playground AI: ${aiResponseText.substring(0, 100)}${aiResponseText.length > 100 ? "..." : ""}`,
						]);
				...
				if (script.scriptContent) {
					...
					(async () => {
						try {
							...
							setActionHistory((prev) => [
								...prev,
								`🔄 Loading script from server: ${script.name}...`,
							]);

							const service = await createDirectWorkshopService();
							if (!service) {
								throw new Error(
									"Workshop service not configured. Please configure Workshop credentials in the extension settings.",
								);
							}
							...
							setActionHistory((prev) => [
								...prev,
								`❌ Failed to load/execute script: ${errorMsg}`,
							]);
							setIsLoadingScript(false);
						}
					})();
				...
				if (script.scriptContent) {
					...
					setActionHistory((prev) => [
						...prev,
						`▶️ Executing Google Recorder script...`,
					]);

					console.log("[PANEL] 🚀 Scheduling script execution in 500ms...");
					// Trigger script execution with the content directly
					setTimeout(async () => {
	...
	const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
		...
		console.log("[PanelApp] File selected:", file.name);
		...
		reader.onload = (e) => {
			try {
				...
				console.log("[PanelApp] File content length:", content?.length);
				console.log("[PanelApp] JSON Format:", jsonFormat);
				...
				// Validate the JSON is parseable and has correct format
				const parsed = JSON.parse(content);
				console.log("[PanelApp] Parsed JSON:", parsed);
				
				if (jsonFormat === "playwright") {
					// Validate Playwright format
					if (!parsed.meta || !parsed.steps) {
						throw new Error("Invalid Playwright script format. Missing 'meta' or 'steps'.");
					}
				} else {
					// Validate Google Recorder format
					if (!parsed.title || !parsed.steps || !Array.isArray(parsed.steps)) {
						throw new Error("Invalid Google Recorder script format. Missing 'title' or 'steps' array.");
					}
				}

				console.log("[PanelApp] Validation passed, setting scriptJson");
				...
				console.error("[PanelApp] Error loading file:", error);
				// Clear scriptJson to keep button disabled
				setScriptJson("");
				setActionHistory((prev) => [
					...prev,
	...
	const executeScript = async (content: string) => {
		...
		// Check if first action is a navigate - if so, use that URL for the new tab
		// This prevents opening blank tab + navigate (which looks like 2 tabs)
		const firstAction = actions[0];
		const initialUrl =
			firstAction?.type === "navigate" && firstAction.url
				? firstAction.url
				: "about:blank";

		// Create a single new tab for script execution
		...
		// Skip first action if it was a navigate (already executed by tab creation)
		const startIndex =
			firstAction?.type === "navigate" && firstAction.url ? 1 : 0;
	...
	const handleRunCommand = async () => {
		...
		const urlPattern =
			/(https?:\/\/[^\s]+)|(?:(?:go\s+to|navigate\s+to|open)\s+)([\w-]+(?:\.[\w-]+)+(?:\/[^\s]*)?)/i;
		...
		try {
			...
			if (activeAutomationTabId) {
				...
				if (tab) {
					addToHistory(
						`🎯 Using tracked tab (ID: ${activeAutomationTabId})`,
					);
					// Make sure it's active
					await chrome.tabs.update(activeAutomationTabId, {
						active: true,
					});
				}
			}
			...
			while (!isDone && stepCount < MAX_STEPS) {
				...
				if (currentTab && currentTab.url !== previousUrl) {
					...
					const tabIdToMonitor =
						activeAutomationTabId || currentTab.id;
					while (loadTimeout > 0) {
						const tabs = await chrome.tabs.query({});
						const tab = tabs.find((t) => t.id === tabIdToMonitor);
						if (tab && tab.status === "complete") {
				...
				if (action.type === "navigate") {
					...
					if (newTab && newTab.id) {
						...
						throw new Error(
							"Failed to create new tab for navigation",
						);
					}
				...
				{mode === "llm" && (
					<div className="command-section">
						{!hasSettings && (
							...
							>
								⚠️ API keys not configured. Please go to Settings
								to set up your Workshop API credentials to use
								LLM mode.
							</div>
						)}
					...
					<div className="playground-monitor-section">
						...
						{playgroundLastResponse && (
							<div className="playground-submitted">
								<strong>
									Latest AI Response (
									{playgroundLastResponse.modelName}):
								</strong>
								<div className="playground-text">
