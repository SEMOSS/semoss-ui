	...
	const handleRunCommand = async () => {
		...
		try {
			...
			while (!isDone && stepCount < MAX_STEPS) {
				...
				let domResponse: {
					success: boolean;
					stats?: { interactiveElements: number };
					elementCount?: number;
					html?: string;
				...
				interface WindowWithMapping extends Window {
					__elementMapping?: Record<string, string>;
				}
				(window as WindowWithMapping).__elementMapping =
					elementMapping || {};
	...
	const executeAction = async (
		...
		try {
			...
			const executeResponse = await chrome.runtime.sendMessage({
				...
				payload: payload,
				elementMapping:
					(window as WindowWithMapping).__elementMapping || {}, // Pass element mapping
			});
