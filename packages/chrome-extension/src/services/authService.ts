...
export class AuthService {
	...
	/**
	 * Clone portal template from the small Playwright portal repo to the project
	 * (uses OAuth session).
	 */
	static async clonePortalsToProject(projectId: string): Promise<void> {
		const expression = `GitCloneIntoProjectPortals(project=["${escapePixelString(projectId)}"], repo=["https://github.com/ibrahimelnemr/playwright-portal-template.git"], branch=["main"], subdirectory=["portals"]);`;

		try {
			const output = await SemossClient.runPixel(expression);
			console.log("[clonePortalsToProject] Response:", output);
		} catch (error) {
			if (error instanceof Error) {
