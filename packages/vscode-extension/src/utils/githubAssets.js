 ...
 */
export async function processGithubAssets(githubLink, downloadsDir, appName, unzipDir) {
    if (!githubLink) return false;

    let zipPath = '';
    let extractPath = '';

    try {        // Parse GitHub link - handles URLs with or without protocol
        const match = githubLink.match(/(?:https?:\/\/)?(?:www\.)?github\.com\/([^\/]+)\/([^\/]+)(?:\/tree\/([^\/]+)\/(.+))?/);
        if (!match) {
            throw new Error('Invalid GitHub link format. Expected format: github.com/user/repo');
        }
        ...
        extractPath = path.join(downloadsDir, `${appName}_github_unzipped_${Date.now()}`);

        vscode.window.showInformationMessage(`Downloading repository from ${zipUrl}...`);
        ...
        // Check if there's an assets subfolder in the GitHub repo
        const repoAssetsDir = path.join(finalDir, 'assets');
        const sourceDir = fs.existsSync(repoAssetsDir) && fs.lstatSync(repoAssetsDir).isDirectory()
            ? repoAssetsDir
            : finalDir;
        ...
        // Copy all files and folders from sourceDir to assetsDir
        await new Promise((resolve, reject) => {
            ncp(sourceDir, assetsDir, function (err) {
                if (err) reject(err);
                else resolve();
            });
        });

        vscode.window.showInformationMessage(`Successfully copied assets from GitHub repository.`);

        // Clean up: delete the zip file and extracted folder
        try {
            if (fs.existsSync(zipPath)) {
                fs.unlinkSync(zipPath);
                vscode.window.showInformationMessage(`Deleted temporary GitHub zip file.`);
            }

            if (fs.existsSync(extractPath)) {
                fs.rmSync(extractPath, { recursive: true, force: true });
                vscode.window.showInformationMessage(`Deleted temporary GitHub extracted folder.`);
            }
        } catch (cleanupErr) {
            vscode.window.showWarningMessage(`Warning: Failed to clean up temporary files: ${cleanupErr.message}`);
            // Continue anyway since the assets are already copied
        }

        return true;
    } catch (err) {
        vscode.window.showErrorMessage(`Error processing GitHub assets: ${err.message}`);
...
/**
 * Helper function to download a zip file with redirect support
 */
export function downloadZipWithRedirect(url, dest) {
