 ...
 */
async function getUserInput(prompt, placeholder = '') {
 ...
 */
async function getStoredInstances(context) {
    const instancesJson = await context.secrets.get('SEMOSS_INSTANCES') || '{}';
    try {
 ...
 */
async function storeInstance(context, alias, instanceData) {
 ...
 * @param {vscode.ExtensionContext} context - The extension context
 */
async function storeSecrets(context) {
    const instances = await getStoredInstances(context);
 ...
 */
async function selectInstance(context) {
    const instances = await getStoredInstances(context);
    const aliases = Object.keys(instances);
    
    if (aliases.length === 0) {
        vscode.window.showWarningMessage('No stored instances found. Please authorize a new instance first.');
        return false;
    }
 ...
 */
async function getCurrentInstance(context) {
 ...
 * @param {vscode.ExtensionContext} context - The extension context
 */
async function removeInstance(context) {
    const instances = await getStoredInstances(context);
    const aliases = Object.keys(instances);
