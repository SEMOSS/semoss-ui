...
class SemossChatbot {
	...
	init() {
		...
		document.body.classList.add(
			this.state.viewport.isMobile ? "mobile-device" : "desktop-device",
		);
	 ...
	 */ bindEvents() {
		...
		if (this.elements.downloadManualBtn) {
			this.elements.downloadManualBtn.addEventListener("click", () =>
	...
	handleResponse(message) {
		...
		if (
			...
			message.text &&
			message.text.includes("removed successfully")
		) {
		...
		return (
			(message.text &&
				(message.text.includes(
					"Project zipped as assets.zip successfully!",
				) ||
					message.text
						.toLowerCase()
						.includes("deployed successfully") ||
	...
	handleInstanceAliasesForRemoval(message) {
		...
		const backBtn = this.createBackButton(() => {
			this.appendMessage(
				"Instance removal cancelled. No instances were removed.",
				"bot",
			);
			this.showOptions();
		});
	...
	appendMessage(text, from, saveToHistory = true, status = null) {
		const { isMobile, isSmallPhone, isLandscape, width } =
			this.state.viewport;
		...
		msgDiv.setAttribute(
			"aria-label",
			`${from === "user" ? "You" : "Bot"}: ${typeof text === "string" ? text.replace(/<[^>]*>/g, "") : "Message"}`,
		);
	...
	updateChatHeaderVisibility() {
		const shouldShow =
			this.state.chatStarted || this.elements.chat.children.length > 0;
	...
	renderOptions(options) {
		...
		options.forEach((opt) => {
			...
			if (opt.icon) {
				if (
					btn.firstChild &&
					btn.firstChild.className === "option-content"
				) {
	...
	async handleOptionClick(option) {
		...
		switch (command) {
			...
			case "semoss.removeInstance":
				this.vscode.postMessage({
					type: "getInstanceAliasesForRemoval",
				});
				return;
		}
	...
	showInstanceSelection(aliases) {
		...
		const backBtn = this.createBackButton(() => {
			this.appendMessage(
				"Instance selection cancelled. No changes were made.",
				"bot",
			);
			this.showOptions();
		});
	...
	getRequiredInputs(command) {
		const inputConfigs = {
			...
			"semoss.authorize": [
				...
				{
					name: "url",
					label: "Semoss Instance URL",
					placeholder: "Enter only the part before /semoss",
					required: true,
				},
				{
					name: "accessKey",
	...
	generateInputHTML(input) {
		...
		if (input.conditional) {
			inputHTML +=
				' conditional-field" data-depends-on="' +
				input.conditional +
				'" style="display: none;"';
		}
		inputHTML += '">';
	...
	showRemoveInstanceConfirmation(alias) {
		...
		yesBtn.style.background =
			"linear-gradient(135deg, var(--color-danger), #ff6b6b)";

		const noBtn = this.createOptionButton("No", () => {
			this.appendMessage(
				`Cancelled removal of instance "${alias}".`,
				"bot",
			);
			this.showOptions();
		});
	...
	applyViewportAdjustments() {
		const { isMobile, isTablet, isLandscape, isSmallPhone, height } =
			this.state.viewport;
		...
		if (this.elements.chat) {
			if (isMobile) {
				const headerHeight = this.elements.chatHeader
					? this.elements.chatHeader.offsetHeight
					: 0;
				let inputAreaHeight = this.elements.inputArea
					? this.elements.inputArea.offsetHeight
					: 0;

				// Default input height if not yet rendered
				if (inputAreaHeight < 10)
					inputAreaHeight = isSmallPhone ? 60 : 80;

				// Additional padding adjustment
				const paddingAdjustment = isSmallPhone
					? 30
					: isLandscape
						? 40
						: 60;
				...
				const adjustedHeight =
					height - headerHeight - inputAreaHeight - paddingAdjustment;
