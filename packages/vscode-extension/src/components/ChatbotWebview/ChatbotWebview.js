...
const {
	...
	mapMessageToCommand,
} = require("./ChatbotSeparateManager.js");
const {
	getSecrets,
	getStoredInstances,
	storeInstance,
...
class SemossChatbotViewProvider {
	...
	resolveWebviewView(webviewView) {
		...
		webviewView.webview.onDidReceiveMessage(async (msg) => {
			...
			if (
				msg.type === "openExternalResource" &&
				msg.resource === "user-manual"
			) {
			...
			if (msg.type === "getInstanceAliases") {
				...
				for (const alias of aliases) {
					urls[alias] =
						instances[alias] && instances[alias].semossUrl
							? instances[alias].semossUrl
							: "";
				}
				// Get current instance alias
				const currentAlias = await this._context.secrets.get(
						...
						if (msg.inputs) {
							switch (command) {
								case "semoss.authorize": {
									...
									try {
										...
										if (
											...
											storedInstance.accessKey !==
												accessKey ||
											storedInstance.privateKey !==
												privateKey
										) {
											throw new Error(
												"Failed to verify stored instance data",
											);
										}
								...
								case "semoss.createNewApp": {
									...
									console.log(
										"ChatbotWebview received inputs:",
										{
											appName,
											description,
											githubLink,
									...
									try {
										...
										if (!secrets) {
											resultMsg =
												"Please authorize an instance first.";
											webviewView.webview.postMessage({
												type: "response",
												status: "error",
												text: resultMsg,
									...
									if (folderCommands.includes(command)) {
										...
										if (!uri) {
											if (
												...
												resultMsg =
													"No workspace folder found."; // Always hide loading even on error
												webviewView.webview.postMessage(
													{
														type: "response",
														status: "error",
			...
			if (msg.type === "removeInstanceByAlias") {
				...
				if (!instances[alias]) {
					webviewView.webview.postMessage({
						type: "response",
						status: "error",
						text: `Instance "${alias}" not found.`,
						hideLoading: true,
					});
					this._addMessageToHistory(
						`Instance "${alias}" not found.`,
						"bot",
						"error",
					);
					return;
				...
				webviewView.webview.postMessage({
					type: "response",
					status: "success",
					text: `Instance "${alias}" removed successfully!`,
					hideLoading: true,
				});
				this._addMessageToHistory(
					`Instance "${alias}" removed successfully!`,
					"bot",
					"success",
				);
				return;
...
function registerChatbotWebview(context) {
	...
	vscode.window.showInformationMessage(
		"If you do not see the Semoss Chatbot sidebar, click the Semoss Chatbot icon in the Activity Bar or use the command: Semoss: Open Chatbot.",
	);
}
