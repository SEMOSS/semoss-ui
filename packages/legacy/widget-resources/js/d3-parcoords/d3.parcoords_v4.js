...
//============================================================================================
var globalBrushed, //used for additional tools filtering
    globalOrder; //used for keeping track of order of dimensions
  ...
  d3.keys(state).forEach(function(key) {
      obj[key] = function(x) {
        ...
        if (key === 'dimensions' && Object.prototype.toString.call(x) === '[object Array]') {
          ...
          if (globalOrder) {
            globalOrder.push(x[globalOrder.length]);
          }
  ...
  g.style("cursor", "move")
    .call(d3.drag()
      ...
      .on("drag", function(d) {
        ...
        pc.sortDimensions();
        xscale.domain(pc.getOrderedDimensionKeys());
        globalOrder = pc.getOrderedDimensionKeys();
        pc.render();
...
pc.selected = function() {
  ...
  for (var k = 0; k < nodes.length; k++) {
    if (d3.brushSelection(nodes[k]) !== null) {
      ...
      var dataType   = typeof __.dimensions[nodes[k].__data__].yscale.domain()[0];
        ...
        for (var i = 0; i < ranger.length; i++) {
          if (actives.includes(nodes[k].__data__) && __.flipAxes.includes(nodes[k].__data__)) {
            if (dataType === 'number') {
              ...
              values.push(__.dimensions[nodes[k].__data__].yscale.invert(ranger[i]).getTime());
            }
          } else {
            if (__.dimensions[nodes[k].__data__].yscale() !== 1) {
              if (dataType === 'number') {
                ...
                values.unshift(__.dimensions[nodes[k].__data__].yscale.invert(ranger[i]).getTime())
              }
            }
          }
        }
