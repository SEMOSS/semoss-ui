...
export default class JDeploy extends Command {
    static description = "Run reactor 1+1, zip current directory, delete assets, and upload the zip";
...
`,
    ...
    static flags = {
        ...
        superVerbose: Flags.boolean({
            char: "s",
            description: "Enable super verbose output (includes all debug info)",
        }),
    ...
    public async run(): Promise<void> {
        ...
        if (flags.showEnv) {
            ...
            this.log(`   • ACCESS_KEY: ${Env.ACCESS_KEY ? '***' + Env.ACCESS_KEY.slice(-4) : 'Not set'}`);
            this.log(`   • SECRET_KEY: ${Env.SECRET_KEY ? '***' + Env.SECRET_KEY.slice(-4) : 'Not set'}`);
        }
        ...
        const tasks = new Listr<{
        ...
        }>([
            {
                ...
                task: async () => {
                    ...
                    if (flags.verbose || flags.superVerbose) {
                        logWithTiming("🔧 Initializing Semoss Insight...", startTime);
                    }
                    ...
                    if (flags.verbose || flags.superVerbose) {
                        logWithTiming("✅ Insight initialized successfully", startTime);
                    }
            ...
            },
            {
                ...
                task: async (context) => {
                    ...
                    if (flags.showRaw || flags.superVerbose) {
                        logWithTiming(`📊 Raw pixelReturn: ${JSON.stringify(pixelReturn, null, 2)}`, startTime);
                    }
                    ...
                    if (flags.verbose || flags.superVerbose) {
                        logWithTiming(`✅ 1+1 Result: ${context.result}`, startTime);
                    }
            ...
            },
            {
                ...
                task: async (context) => {
                    ...
                    if (flags.verbose || flags.superVerbose) {
                        logWithTiming(`📦 Zipping current directory`, startTime);
                    }
                    ...
                    try {
                        ...
                        if (flags.verbose || flags.superVerbose) {
                            logWithTiming(`📁 Found ${paths.length} files to zip`, startTime);
                        }

                        if (flags.superVerbose) {
                            this.log(`📋 Files to include:`);
                            paths.slice(0, 10).forEach(p => this.log(`   • ${p}`));
                            if (paths.length > 10) {
                                this.log(`   • ... and ${paths.length - 10} more files`);
                            }
                        }
                        ...
                        // Convert to a buffer
                        context.zipBuffer = await zip.toBufferPromise() || "NOT ANYTHING";

                        if (flags.verbose || flags.superVerbose) {
                            logWithTiming(`✅ Zip created successfully (${context.zipBuffer.length} bytes)`, startTime);
                        }

                        if (flags.superVerbose) {
                            this.log(`🔍 Zip Analysis:`);
                            this.log(`   • Files included: ${paths.length}`);
                            this.log(`   • Zip size: ${context.zipBuffer.length} bytes`);
                            this.log(`   • Execution Time: ${Date.now() - startTime}ms`);
                        }

                    } catch (error) {
                        if (flags.verbose || flags.superVerbose) {
                            logWithTiming(`❌ Zip creation failed: ${error}`, startTime);
                        }
                        throw error;
                    }
            ...
            // },
            {
                ...
                task: async (context) => {
                    ...
                    if (!context.zipBuffer) {
                        if (flags.verbose || flags.superVerbose) {
                            logWithTiming("⏭️ No zip buffer to upload, skipping upload step", startTime);
                        }
                        return true;
                    }

                    if (flags.verbose || flags.superVerbose) {
                        logWithTiming("📤 Uploading zipped directory to the server", startTime);
                    }

                    if (flags.superVerbose) {
                        this.log(`🔍 Upload Details:`);
                        this.log(`   • Zip Buffer Size: ${context.zipBuffer.length} bytes`);
                        this.log(`   • Target App: ${Env.APP}`);
                        this.log(`   • Target Path: version/assets/`);
                    }
                    ...
                    try {
                        ...
                        const file = new File([context.zipBuffer], fileName) as any;

                        if (flags.superVerbose) {
                            this.log(`🔍 File Details:`);
                            this.log(`   • File Name: ${fileName}`);
                            this.log(`   • File Size: ${context.zipBuffer.length} bytes`);
                        ...
                        this.log(`✅ Upload of ${fileName} completed., ${JSON.stringify(uploaded)}`);

                        if (flags.showRaw || flags.superVerbose) {
                            logWithTiming(`📊 Upload result: ${JSON.stringify(uploaded, null, 2)}`, startTime);
                        }

                        // Unzip the uploaded file
                        await insight.actions.run(
                            `UnzipFile(filePath=["version/assets/test/${uploaded[0].fileName}"], space=["${Env.APP}"])`,
                        );

                        // Clean up the zip file
                        await insight.actions.run(
                            `DeleteAsset(filePath=["version/assets/test/${uploaded[0].fileName}"], space=["${Env.APP}"])`,
                        );

                        context.uploadResult = uploaded[0];

                        if (flags.verbose || flags.superVerbose) {
                            logWithTiming(`✅ Upload completed: ${uploaded[0].fileName}`, startTime);
                        }

                        if (flags.superVerbose) {
                            this.log(`🔍 Upload Analysis:`);
                            this.log(`   • File Name: ${uploaded[0].fileName}`);
                            this.log(`   • File Location: ${uploaded[0].fileLocation}`);
                            this.log(`   • Execution Time: ${Date.now() - startTime}ms`);
                        }

                    } catch (error) {
                        if (flags.verbose || flags.superVerbose) {
                            logWithTiming(`❌ Upload failed: ${error}`, startTime);
                        }
                        throw error;
                    }
        ...
        tasks
            ...
            .then((context) => {
                ...
                if (context.uploadResult !== undefined) {
                    this.log(`📤 Upload Result: ${context.uploadResult.fileName}`);
                }

                if (flags.verbose || flags.superVerbose) {
                    this.log("\n📋 Summary:");
                    this.log(`   • 1+1 calculation: ${context.result}`);
                    this.log(`   • Zip operation: ${context.zipBuffer ? "Completed" : "Failed"}`);
                    this.log(
                ...
                if (flags.superVerbose) {
                    ...
                    this.log(`   • Successful Operations: ${[
                    ...
                    this.log(`   • Debug Mode: ${flags.debug ? 'Enabled' : 'Disabled'}`);
                    this.log(`   • Verbose Mode: ${flags.verbose ? 'Enabled' : 'Disabled'}`);
                    this.log(`   • Super Verbose: ${flags.superVerbose ? 'Enabled' : 'Disabled'}`);
                    this.log(`   • Show Raw Data: ${flags.showRaw ? 'Enabled' : 'Disabled'}`);
                    this.log(`   • Show Timing: ${flags.showTiming ? 'Enabled' : 'Disabled'}`);
                }
            })
            .catch((err) => {
                // log the error
