 ...
 */
export const ResponseMessageThinking: React.FC<
	ResponseMessageThinkingProps
> = ({ part, status, isFirstView }) => {
	...
	const displayedThinking = isStreaming ? typewriter.rendered : part.thinking;
	...
	const canToggleExpansion = !isStreaming && (isOverflowing || isExpanded);
	...
	return (
		<div
			className={`relative mb-2 rounded-lg border border-border text-muted-foreground text-sm shadow-sm ${
				effectiveExpanded || !canToggleExpansion ? "" : "cursor-pointer"
			}`}
		>
