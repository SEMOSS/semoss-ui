...
export const ResponseMessage: React.FC<ResponseMessageProps> = ({
	...
	const { root } = useRoot();
	const parts = useStore(message, (s) => s.parts);
	const isThinking = useStore(message, (s) => s.isThinking);
	const feedback = useStore(message, (s) => s.feedback);
	...
	const feedbackTextEnabled =
		!!root.getState().theme.featureFlags?.enableFeedbackText;
	...
	const recordFeedback = async (rating: boolean) => {
		const isDeleting = feedback?.rating === rating;
	...
	const submitFeedbackText = async () => {
		...
		try {
			await message.recordFeedback(pendingRating, feedbackText.trim());
			toast.success(t("notifications.feedbackSuccess"));
			setIsFeedbackTextOpen(false);
	...
	const copyImage = async () => {
		const mediaPart = parts.find((p) => p.type === "MEDIA");
		if (
			!mediaPart ||
		...
		<div className="group">
			<div className="mb-0 flex w-full flex-col gap-2 pe-3 sm:pe-10">
				{parts.map((p, pIdx) => {
					...
					if (p.type === "TEXT") {
						...
						const { Icon, ext } = getExtIcon(p.mediaInfo.fileName);
						const isImage =
				...
				{message.hasUnfinishedTools && !isThinking && (
			...
			{message.id !== STREAMING_PLACEHOLDER_ID && (
				<div className="flex flex-row items-center gap-0.5 pt-2">
					...
					{root.getState().theme.featureFlags?.enableRewrite &&
						parentHasContent && (
							<Tooltip>
								<TooltipTrigger asChild>
									<Button
										disabled={!inputMessage?.parent?.parent}
										variant="ghost"
										size="icon"
										onClick={() => {
											rewriteMessage();
