...
export const RoomFileExplorer: React.FC<RoomFileExplorerProps> = ({
	layout,
	room,
	node,
}) => {
	const roomInsightId = useStore(room, (s) => s.insightId);
	...
	const removeTabsForPath = (deletedPath: string, isDirectory: boolean) => {
		...
		<div className="h-full w-full text-foreground">
			<FileExplorer
				...
				onItemsDeleted={(items) => {
					items.forEach((item) => {
						removeTabsForPath(item.path, item.type === "directory");
					});
				}}
							...
							secondaryActions={[
								...
								},
								{
									...
									action: async (item) => {
										...
										// download the file
										await download(roomInsightId, fileKey);
