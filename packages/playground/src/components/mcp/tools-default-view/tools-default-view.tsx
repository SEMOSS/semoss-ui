...
export const ToolsDefaultView = ({
	...
	const [response, setResponse] = useState<string | undefined>(
		toolStatus === "SUCCESS" ? tool.response : undefined,
	);
	...
	const showResponse = toolStatus === "SUCCESS";
	const toolFailed =
		toolStatus === "ERROR" ||
		toolStatus === "CANCELLED" ||
		toolStatus === "PAUSED";
	const requiredFields = Object.entries(properties).filter(([fieldName]) =>
	...
	const handleSubmit = async () => {
		...
		if (m && m instanceof ResponseMessageStore && toolIsOpen) {
			room.processTool(
				m.id,
				tool.id,
				output,
	...
	const renderFields = (fields: [string, FieldSchema][], required: boolean) =>
