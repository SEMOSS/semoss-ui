...
export async function searchPlaywrightRecordings(
	...
	try {
		...
		if (projectsRes.pixelReturn[0].operationType?.includes("ERROR")) {
			throw new Error(
				`Failed to fetch projects: ${projectsRes.pixelReturn[0].output}`,
			);
		}
		...
		for (const project of projects) {
			try {
				...
				const recordingsRes = await runPixel<[string[]]>(
					recordingsPixel,
					insightId,
				);

				if (
					recordingsRes.pixelReturn[0].operationType?.includes(
						"ERROR",
					)
				) {
					continue;
				...
				console.error(
					`Error fetching recordings from ${project.project_name}:`,
					err,
				);
			}
		}
		...
		return allRecordings.filter((recording) => {
			const titleMatch = recording.title
				?.toLowerCase()
				.includes(lowerQuery);
			const descriptionMatch = recording.description
				?.toLowerCase()
				.includes(lowerQuery);
			const intentMatch = recording.intent
				?.toLowerCase()
				.includes(lowerQuery);
...
export async function getPlaywrightRecordingSteps(
	...
	try {
		...
		const stepsRes = await runPixel<
			[{ steps?: Record<string, any[]>; intent?: string }]
		>(stepsPixel, insightId);

		if (stepsRes.pixelReturn[0].operationType?.includes("ERROR")) {
			throw new Error(
				`Failed to fetch steps: ${stepsRes.pixelReturn[0].output}`,
			);
		}
