...
export const PromptLibraryDialog: React.FC<PromptLibraryDialogProps> = ({
	open,
	onOpenChange,
	prompts,
	isLoading = false,
	...
	// All unique tags across all prompts for the filter bar
	...
	// Group by tag (collapse to single group when a tag filter is active)
					...
					{allTags.length > 0 ? (
						<div className="mt-2 flex flex-wrap gap-1.5">
							<button
								...
								className={`rounded-full border px-2.5 py-0.5 text-xs transition-colors hover:bg-accent ${activeTag === null ? "border-primary bg-primary text-primary-foreground" : ""}`}
							>
								All
							</button>
							{allTags.map((tag) => (
								<button
									...
									className={`rounded-full border px-2.5 py-0.5 text-xs transition-colors hover:bg-accent ${activeTag === tag ? "border-primary bg-primary text-primary-foreground" : ""}`}
								>
									{tag}
								</button>
							))}
