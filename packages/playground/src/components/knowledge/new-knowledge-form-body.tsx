 ...
 */
export const NewKnowledgeFormBody = ({
	formId,
	onSuccess,
	onLoadingChange,
}: NewKnowledgeFormBodyProps) => {
	...
	const showEmbeddingOptions =
		root.getState().theme.featureFlags?.allowEmbeddingOptions;
	const defaultEmbedderId = root.getState().theme.defaultEmbedderId ?? "";
	const runMCP = root.getState().theme.featureFlags?.enableKnowledgeMCP;
	...
	const submitForm = async () => {
		try {
			...
			const engineId = createVectorEngine.pixelReturn[0].output.engine_id;
			if (!engineId) {
	...
	return (
		...
		>
			<FieldGroup>
				...
				<Field>
					<FieldLabel>{t("knowledge:form.filesLabel")}</FieldLabel>
					<Input
