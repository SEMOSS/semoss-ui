...
export default function App() {
	...
	return (
		...
		>
			{/* Toolbar row */}
			<Box className="flex min-h-14 flex-wrap items-center gap-2 border-line border-b bg-surface px-3 py-2 shadow-black/20 shadow-lg">
				<BrowserToolbar
				...
				<Box className="ml-auto flex min-w-0 flex-wrap items-center justify-end gap-1.5">
					...
					{isPlaybackPaused && (
						<span className="rounded-md border border-warning/40 bg-warning/10 px-2 py-1 font-semibold text-warning text-xs">
							Paused
						</span>
					)}
					{isRunningRecording && (
						<span className="rounded-md border border-accent/40 bg-accent/10 px-2 py-1 font-semibold text-accent text-xs">
							Step {runningStepId ?? ""}
						</span>
					)}
				</Box>
			...
			<Box className="flex min-h-0 flex-1">
				...
				<Box
					className="border-line border-l bg-surface"
					sx={{
						width:
